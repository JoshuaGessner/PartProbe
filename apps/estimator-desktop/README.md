...
The current shell can choose one local STEP, STL, or 3MF file and, when an external pinned developer runtime is explicitly configured and passes host-side verification, request provisional geometry facts through `DraftEstimateApplication`. The native host retains the selected path and complete `DraftEstimateSession` only for the running process. The WebView receives a session token, leaf filename, hashes, sanitized stage codes, and provisional measurements; it never receives the path or CAD bytes. Exact STEP analysis can continue into the estimate workflow, which requires explicit unit/warning review, every manual estimate value, five confirmed session rates, and a confirmed versioned pricing policy before the native application session can return a deterministic selling price and trace. Mesh estimates remain unavailable. Analysis can be cancelled cooperatively. The primary Estimate view keeps reusable inputs behind Settings and collapses the temporary manual manufacturing assumptions. USD rate/pricing drafts and one optional typed material/offer/stock/machine/runtime starter bundle now persist locally as immutable settings revisions; models, analyses, estimates, approvals, and quotes are not saved or uploaded.
