...
- Bounded TASK-004 ASCII/binary STL and in-memory 3MF comparison analyzers with shared deterministic mesh measurements, twelve governed synthetic fixtures, persisted deterministic packages for every Core 3MF unit declaration and the normative default, ZIP/OPC/XML/entity limits, and `partprobe-3mf-spike-v3` direct or single-reference-component transform/provenance plus bounded model-metadata presence evidence. Metadata names/values are not retained or interpreted, and the packages remain alternate encodings of one analytic cube; these are internal mesh-only spikes, not supported importers.
