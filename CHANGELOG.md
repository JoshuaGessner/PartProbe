...
- Twenty deterministic public adversarial mesh inputs with typed failure expectations: eighteen 3MF packages cover branching, non-immediate and forward component references, unused component objects, build unions, object/item/vendor metadata, triangle material attributes, required extensions, relationship traversal, case-ambiguous parts, excessive compression ratio, unsupported compression, encryption, absolute/backslash entry names, and directory entries; two binary STL files cover truncated framing and unsupported facet attributes. Each pins an exact sanitized diagnostic and forbids geometry evidence; this is parser-level rejection coverage, not worker/desktop or production-importer evidence.
