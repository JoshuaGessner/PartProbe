...
- Thirty-one deterministic public adversarial mesh inputs with typed failure expectations: twenty-three 3MF packages cover graph/structure/metadata/material/extension/archive/transform policy, malformed XML, forbidden document types, and entry-count limits; four binary STL files cover truncated framing, unsupported facet attributes, non-finite normals, and triangle-count limits; and four ASCII STL streams cover invalid UTF-8, malformed facet grammar, an empty solid, and degenerate geometry. Each pins an exact sanitized diagnostic and forbids geometry evidence; this is parser-level rejection coverage, not worker/desktop or production-importer evidence. Backslash-name package construction rewrites equal-length raw ZIP filename fields so the governed bytes reproduce independently of host path separators.
