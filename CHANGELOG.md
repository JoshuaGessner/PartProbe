...
- Bounded TASK-004 ASCII/binary STL and in-memory 3MF comparison analyzers with shared deterministic mesh measurements, eleven governed synthetic fixtures, persisted deterministic packages for every Core 3MF unit declaration and the normative default, ZIP/OPC/XML/entity limits, and `partprobe-3mf-spike-v2` direct or single-reference-component transform/provenance evidence. The unit packages are alternate encodings of one analytic cube; these remain internal mesh-only spikes, not supported importers.
