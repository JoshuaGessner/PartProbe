...
76. The TASK-004 3MF comparison slice supports one direct mesh object and one build item only. Preserve the explicit unit versus normative-default distinction, apply the 3MF build transform before canonical-millimetre conversion, retain mesh provenance/counts, and fail visibly on components, multiple objects/build items, metadata/material extensions, or other structures until their interpretation and fixtures are governed. Never silently flatten them.
