...
76. The TASK-004 3MF comparison v2 slice supports either one direct mesh object or exactly one additional component object that references that mesh once, plus one build item. Enforce explicit object/component limits; retain absent versus present component IDs/transforms; apply the component transform before the build transform and canonical-millimetre conversion; and reject singular/reflected transforms. Multiple component instances, nested component graphs, extra objects/build items, metadata/material extensions, unions, and other structures remain visible failures until their interpretation and fixtures are governed. Never silently flatten them.
