...
65. Linux desktop package evidence must use an explicit supported baseline, pinned build tools, reviewed Tauri prerequisites, bundled local frontend content, package inspection, and a bounded virtual-display launch. Keep the workflow opt-in and non-publishing; reject package formats whose build path fetches unpinned helper tools until exact sources/hashes are governed. An unsigned package build or headless window smoke is not signed distribution, native-OCCT integration, installer acceptance, accessibility acceptance, or production support.
