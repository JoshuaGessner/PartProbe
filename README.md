...
|---|---|---|
| Calculations | Typed money/units, deterministic rules, itemized traces, versioned snapshots/replay, and a headless GUI-2 session service that composes explicit model/manual/rate/policy inputs | No completed desktop review/input/rate/result workflow, durable estimate, shop calibration, or approved quote workflow |
...
| Desktop and storage | GUI-2 supplies the headless application use case; GUI-3 supplies the restrictive shell; GUI-4 now adds an async, token-only analysis command, explicit developer-worker configuration, native session retention, and path-free provisional geometry facts | No review/input/rate/result forms, cancellation command, durable audit/database, save/reopen workflow, packaged native worker, supported installer, signed release, or three-OS launch evidence exists |

The local macOS default workspace now passes **128 runtime tests** plus one compile-fail doctest and six native-tooling tests; GUI-4 also passes 20 focused contract/native-host/UI tests, strict native/WASM lint, an offline frontend build, and a 1280 × 900 semantic/visual preview. GitHub Actions run 31334097970 passes formatting, tooling, workspace/native-host/WASM lint, all-target tests, and doctests on macOS, Ubuntu, and Windows at GUI-4 implementation commit `8cd2a84`. Twenty-six focused optional-native adapter/worker tests plus strict native-feature Clippy pass locally on Apple Silicon. These results validate foundations and bounded developer contracts; they do not establish production estimating accuracy or release readiness.
...
1. Review the GUI-1 fixture evidence, complete GUI-4's review/manual-input/rate/policy/result and cancellation workflow over the connected native analysis adapter, and continue TASK-003 network/filesystem and target-specific resource containment plus Windows/Linux native construction.
...
The nearest testable GUI is a narrower internal, provisional, session-only STEP slice. GUI-1's native seam awaits fixture review; GUI-2's headless model-to-estimate service, GUI-3's restrictive shell, and GUI-4's native provisional-analysis adapter are implemented. A correctly configured Apple-Silicon developer run can now select a STEP file and request numeric geometry facts, but this machine lacks the local OCCT installation needed for an end-to-end runtime demonstration. The remaining GUI-4 checkpoint adds explicit review, complete manual inputs, pinned rates/policy, deterministic result trace, and cancellation; GUI-5 then adds end-to-end/accessibility evidence. A 3D viewport adds roughly two or three checkpoints because no tessellation/viewer implementation exists. See the [testable GUI vertical-slice plan](docs/07-delivery/gui-vertical-slice-plan.md) and [desktop developer runbook](apps/estimator-desktop/README.md). The first usable cross-platform product slice still requires broader model intake, reviewable measurements, editable assumptions, a guided rate library, transparent estimate/pricing traces, save/reopen, previews, and packaging.
