...
|---|---|---|
| Calculations | Typed money/units, deterministic rules, itemized traces, versioned snapshots/replay, a headless GUI-2 session service, and a GUI-4 desktop workflow that submits explicit model/manual/rate/policy inputs to that service | No durable estimate, shop calibration, model-derived stock/runtime proposal, or approved quote workflow |
| Rates and resources | Empty-on-install, user-owned rate cards; effective dating, scope resolution, approval state, pricing and rounding policies; durable schema-v2 starter Settings plus schema-v3 immutable multi-entry catalog/selection persistence; schema-v4 append-only catalog-decision audit; native repository/policy/audit composition with a shipped deny-all baseline and controlled exact-operator evidence; contract-v7 path-free read-only catalog review in Settings | Persisted or active-for-proposals resources are not calculation authority; no authenticated identity/role source, reviewed shipped allow configuration, catalog editor/activation UI, or proposal resolver exists; PartProbe supplies no production rates |
...
| Desktop and storage | GUI-2 supplies the headless application use case; GUI-3 supplies the restrictive shell; GUI-4/GUI-5 provide an actual Apple-Silicon session-only path through real OCCT geometry, revision-bound review, complete inputs, cancellation/recovery, and a deterministic USD 702 trace. USE-1 makes model intake primary; USE-2 adds host-owned SQLite save/reopen for Settings with optimistic revisions and immutable replay. Ubuntu and Windows pass configured native-host smokes; Ubuntu also passes an unsigned extracted Debian package with verified runtime and exact-payload picker/analysis evidence | Models and estimates remain session-only; no multi-entry Settings editor, model-derived stock/material/runtime proposals, signed native runtime, supported installer, complete accessibility evidence, interactive Windows acceptance, or three-OS native GUI/package evidence exists |

The local workspace passes **239 runtime tests** plus one compile-fail doctest and 74 Python tooling tests. Portable tests cover desktop startup/runtime verification, fixed package-resource selection, immutable Settings/resource/catalog persistence and backup replay, bounded catalog governance, policy/audit-gated catalog activation, schema-v4 durable decision audit/replay/corruption handling, native deny-all/exact-operator policy composition, contract-v7 path-free read-only catalog mapping and overwrite prevention, aggregate workspace-output containment, Windows runtime/DLL/PE contracts, and exact portal-semantic Linux GUI automation; Linux CI additionally exercises parser-phase socket and descendant denial. Configured native Linux run 33546682993 and Windows run 33546698680 pass real-worker STEP/STL/3MF desktop-host evidence. The host resolves a runtime only from the fixed packaged resource child or an explicit developer override. The 174 MiB USE-1 Apple-Silicon app contains a re-verified 72-artifact OCCT/worker runtime and passes the packaged-resource STEP-to-USD-702 smoke, but predates the latest Settings contracts. The shipped catalog policy remains deny-all and no activation command exists. Details and limitations are in [PROJECT_STATE](docs/PROJECT_STATE.md) and [TASK-003 validation evidence](docs/06-quality/task-003-validation.md); these results are internal engineering evidence, not production estimating accuracy or release readiness.
...
2. Finish USE-2 by adding a separately versioned catalog request surface with host-owned identity, exact permissions, and governed editing/activation while retaining deny-all startup until a reviewed identity/role-backed allow configuration exists.
