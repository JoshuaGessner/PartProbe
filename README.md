...
| Geometry worker | Bounded control schema v2/transport manifest v2, explicit verified-copy transport, exact Unix descriptor and Windows HANDLE direct allowlisting with unrelated-resource exclusion, worker-side identity/type/hash/length/quota verification, private workspaces, cancellation grace/acknowledgement, forced termination, aggregate-output monitoring, Linux parser socket/process syscall denial, partial CPU/file/process containment, audit/security seams, and governed derivative handoff | No general filesystem sandbox, macOS/Windows parser egress denial, complete cross-platform resource containment, configured native Linux filter evidence, or durable controlled store |
| STEP/OCCT | Optional OCCT 8.0 Apple-Silicon ABI-v3 spike parses exact verified bytes, emits a source-bound provisional snapshot, has a fail-closed pinned source-build command, measures both an OCCT-generated cube and a manually authored analytic prism, and can assemble the worker plus complete observed OCCT closure into a developer runtime that the desktop re-verifies before launch | Not a supported product importer; formal fixture review, broader accuracy corpus, Windows/Linux native construction/runtime launch, signed distribution, and legal review remain open |
...
The local macOS default workspace passes **143 runtime tests** plus one compile-fail doctest and eleven native-tooling tests. Portable tests cover desktop startup verification and aggregate workspace-output containment; Linux CI additionally exercises parser-phase socket and descendant denial. GUI-5's focused contract/native-host/UI set passes 29 tests plus one opt-in real-worker smoke ignored by default; that smoke passes after in-host verification of the separately assembled runtime, and 26 optional-native adapter/worker tests pass during construction. The rebuilt unsigned app also passes the full native-picker, verified-runtime STEP-to-USD-702 flow using only the runtime-root/workspace configuration; its earlier live-cancellation, safe malformed-input recovery, and review-reset evidence remains valid. [GitHub Actions run 31553615142](https://github.com/JoshuaGessner/PartProbe/actions/runs/31553615142) passes the complete default matrix on macOS, Ubuntu, and Windows at implementation commit `097f78f`; details and limits are recorded in [TASK-003 validation evidence](docs/06-quality/task-003-validation.md). These results validate foundations and one bounded developer configuration, not production estimating accuracy or release readiness.
...
1. Review the GUI-1 fixture evidence; verify the Linux parser filter with native OCCT; and extend the verified developer-runtime construction/launch path to Windows and Linux, dynamic-link/license/SBOM inspection, signed application integration, macOS/Windows parser egress denial, general filesystem confinement, and remaining TASK-003 resource containment.
