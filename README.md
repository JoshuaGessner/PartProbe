...
The nearest testable GUI now exists as a narrow internal, provisional, session-only slice. GUI-2 through GUI-5 have passed their bounded exact-STEP checkpoints on the evidenced Apple-Silicon configuration: the actual unsigned app can select the governed STEP prism, analyze it through freshly constructed OCCT 8.0.0, require explicit inputs, and show a deterministic USD 702 trace; cancellation and malformed-input recovery also work. Contract v4 additionally presents governed STL/3MF facts but deliberately keeps mesh estimating unavailable. The Ubuntu package carries its verified runtime and passes real packaged-resource STEP analysis plus exact native-picker/analysis interaction on one virtual configuration, while the complete estimate-form workflow remains Apple-Silicon evidence. A configured real-worker STL/3MF desktop-host gate is locally compiled and wired into the opt-in Linux/Windows native workflows; its hosted results are pending. The Apple developer run still requires separate runtime/workspace configuration. All work disappears on exit, GUI-1 formal fixture review remains open, and this is not developer alpha or importer support. A 3D viewport adds roughly two or three checkpoints because no tessellation/viewer implementation exists. See the [testable GUI vertical-slice plan](docs/07-delivery/gui-vertical-slice-plan.md), [GUI-5 evidence](docs/06-quality/gui-5-validation.md), and [desktop developer runbook](apps/estimator-desktop/README.md). The first usable cross-platform product slice still requires broader model intake, reviewable measurements, guided durable shop rates, save/reopen, native packaging, and release evidence.
