...
| Rates and resources | Empty-on-install, user-owned rate cards; effective dating, scope resolution, approval state, pricing and rounding policies; durable schema-v2 starter Settings plus schema-v3 immutable multi-entry catalog/selection persistence; schema-v4 append-only catalog-decision audit; headless governed catalog-draft and activation services; native activation repository/policy/audit composition with a shipped deny-all baseline; contract-v7 path-free catalog review, contract-v8 activation-request evidence, and contract-v9 native-owned catalog-draft save mapping | Persisted drafts or active-for-proposals resources are not calculation authority; ordinary startup has no authenticated identity, so catalog draft save and activation are unavailable; no governed multi-entry editor, reviewed shipped allow configuration, activation UI, or proposal resolver exists; PartProbe supplies no production rates |
...
The local workspace passes **255 runtime tests** plus one compile-fail doctest and 74 Python tooling tests. Portable tests cover desktop startup/runtime verification, fixed package-resource selection, immutable Settings/resource/catalog persistence and backup replay, bounded catalog governance, governed immutable catalog Draft editing, policy/audit-gated catalog activation, schema-v4 durable decision audit/replay/corruption handling, native deny-all/exact-operator policy composition, contract-v7 read-only catalog mapping, contract-v8 activation requests, and the contract-v9 path-free native-owned draft-save boundary, aggregate workspace-output containment, Windows runtime/DLL/PE contracts, and exact portal-semantic Linux GUI automation; Linux CI additionally exercises parser-phase socket and descendant denial. Configured native Linux run 33546682993 and Windows run 33546698680 pass real-worker STEP/STL/3MF desktop-host evidence. The host resolves a runtime only from the fixed packaged resource child or an explicit developer override. The 174 MiB USE-1 Apple-Silicon app contains a re-verified 72-artifact OCCT/worker runtime and passes the packaged-resource STEP-to-USD-702 smoke, but predates the latest Settings contracts. Ordinary startup has no native actor, so both catalog commands return explicit unavailability; the shipped activation policy remains deny-all and the WebView exposes neither command. Details and limitations are in [PROJECT_STATE](docs/PROJECT_STATE.md) and [TASK-003 validation evidence](docs/06-quality/task-003-validation.md); these results are internal engineering evidence, not production estimating accuracy or release readiness.
