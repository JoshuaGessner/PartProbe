...
The current repository implements headless calculation/rate foundations; bounded application, security, document-storage, platform, and geometry-worker services; SQLite shop-setting schemas v1–v4; an internal Tauri/Leptos desktop path; and optional three-OS OCCT developer-spike boundaries. VIS-1 adds an isolated `wgpu` renderer plus a developer-gated same-window Metal surface. VIS-2 adds the source/analysis-bound display contract, artifact decoder, worker protocol v2, shared-quota dual-output claim, native OCCT tessellation/emission, application retention, and exact developer-desktop handoff into checked indexed renderer buffers. The WebView remains geometry-free. Source analysis begins without stock; a matching reviewed proposal may add its exact rectangular blank through native-only `proposed-stock-display-placement-v1`, which is centered and source-axis aligned for review while standard size and availability remain unresolved. Complete production estimate coverage, supported product import, authenticated shop authority, signed packages, and release acceptance are not implemented.
