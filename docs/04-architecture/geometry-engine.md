...
The low-level local-source opener now opens read-only and protects the final path component: Unix uses `O_NOFOLLOW`; Windows opens the reparse point itself, rejects the reparse-point attribute, and requests identification-only security quality of service. This closes the supervisor's pathname-reopen window and the final-component link-following case, but it is not the complete asset service or worker sandbox. The application service must still authorize the selected path and safely resolve and contain parent components. The worker still receives a controlled filename copy rather than an inherited descriptor/handle. Direct handle passing where practical, a documented copy fallback, controlled output ownership, and OS access controls remain required.
