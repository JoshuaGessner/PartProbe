...
The current asset-grant spike copies one authorized regular file to the fixed worker-local name `partprobe-input.asset` using create-new semantics. It rejects source symlinks, empty/oversized inputs, pre-existing targets, and SHA-256 mismatch; marks the staged copy read-only; and removes it after worker completion. One opaque capability therefore maps to one staged input in one job directory without entering the protocol as a path. This is not the final descriptor/handle grant: pathname time-of-check/time-of-use hardening and OS access controls remain required.
