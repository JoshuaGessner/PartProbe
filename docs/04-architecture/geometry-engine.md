...
The current supervisor accepts a consumed `AssetReadGrant` that binds one opaque capability to one already-open, nonempty regular-file handle and captures its authorized byte length. The supervisor never receives or reopens the source pathname: it verifies the request capability, rewinds and reads the same handle, rejects length drift/oversize/hash mismatch, then drops the source grant before worker launch. It copies verified bytes to the create-new fixed worker-local name `partprobe-input.asset`, marks that copy read-only, and removes it after completion.

This closes the supervisor's pathname-reopen time-of-check/time-of-use window, but it is not the complete asset service or worker sandbox. The application-service resolver must still open sources read-only with target-appropriate no-follow/reparse-point policy and authorization, and the worker still receives a controlled filename copy rather than an inherited descriptor/handle. Direct handle passing where practical, a documented copy fallback, controlled output ownership, and OS access controls remain required.
