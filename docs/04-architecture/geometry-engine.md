...
The current transport uses bounded newline-framed JSON on worker stdin and one bounded JSON response on stdout. Control-stream schema version 2 requires an `execute` frame carrying request schema version 1 plus asset-transport manifest schema version 1; a later identity-bound `cancel` frame is optional. The supervisor atomically creates a private per-job directory beneath its configured controlled root, launches the executable there with a cleared inherited environment, polls cancellation and wall time, kills and reaps failed workers, and verifies schema, job, and correlation identity before accepting a response. Unix job directories use owner-only mode; Windows inherits the configured root's access-control list. No unrestricted source path, descriptor value, HANDLE value, raw CAD content, database credential, or renderer handle enters JSON.

The supervisor accepts a consumed `AssetReadGrant` that binds one opaque capability to one already-open, nonempty regular-file handle and captures its authorized byte length. It never reopens the source pathname: it verifies the request capability, rewinds and reads the same handle, rejects length drift/oversize/hash mismatch, then drops the source grant before worker launch. For the implemented `verified_private_copy` transport, it copies verified bytes to the create-new fixed worker-local name `partprobe-input.asset`, marks the file read-only, and sends a path-free manifest binding transport, job, correlation, capability, authorized length, and expected SHA-256. The worker independently opens that fixed name with final-component no-follow protection, rechecks all binding fields, regular-file type, exact length, quota, and hash, and retains immutable verified bytes. Native adapters parse those bytes; they cannot substitute or reopen an arbitrary source path. The supervisor removes the copy and private directory on every handled completion/failure path.
...
| `PreferDirect` | Unix descriptor or Windows HANDLE when a whitelist-capable launcher exists | Records `DirectTransportUnavailable`, then uses `verified_private_copy` | Fallback reason is observable in `GeometryWorkerExecution` |
| `RequireDirect` | Exact allowlisted descriptor/HANDLE only | Unavailable on every target | Fails closed with `ASSET_DIRECT_TRANSPORT_UNAVAILABLE`; no silent copy or launch |

Stable `std::process::Command` does not provide a portable whitelist for extra inherited resources, and Windows direct HANDLE launch requires an exact `PROC_THREAD_ATTRIBUTE_HANDLE_LIST`. The project forbids claiming direct isolation until deliberately inheritable unrelated descriptor/HANDLE tests pass. Platform launch mechanics will live behind the `geometry-import` transport policy or a future `platform` adapter; theoretical OS capability, implemented support, and configured deployment mode remain distinct.
