...
The supervisor accepts a consumed `AssetReadGrant` that binds one opaque capability to one already-open, nonempty regular-file handle and captures its authorized byte length. It never reopens the source pathname: it verifies the request capability, rewinds and reads the same handle, and rejects length drift, oversize, or hash mismatch. For `verified_private_copy`, it copies verified bytes to the create-new fixed worker-local name `partprobe-input.asset`, marks the file read-only, and sends a path-free manifest. For Unix direct transport, the platform adapter atomically creates a close-on-exec duplicate, clears close-on-exec only for that descriptor in the child, and marks every other non-stdio descriptor close-on-exec before `exec`; the manifest binds its numeric descriptor ID. The parent drops its duplicate immediately after spawn. In both modes the worker rechecks all binding fields, regular-file type, exact length, quota, and hash into immutable bytes before adapter dispatch. Native adapters cannot substitute or reopen an arbitrary source path. The supervisor removes any copy and the private directory on every handled completion/failure path.
...
| `PreferDirect` | Unix descriptor or Windows HANDLE when a whitelist-capable launcher exists | `unix_descriptor` on Unix; Windows records `DirectTransportUnavailable`, then uses `verified_private_copy` | Fallback reason is observable in `GeometryWorkerExecution` |
| `RequireDirect` | Exact allowlisted descriptor/HANDLE only | `unix_descriptor` on Unix; unavailable on Windows | Windows fails closed with `ASSET_DIRECT_TRANSPORT_UNAVAILABLE`; no silent copy or launch |

The new `platform` crate isolates the Unix launch mechanics and proves with a deliberately inheritable sentinel that only stdio plus the intended descriptor survive `exec`. Stable `std::process::Command` still does not expose the Windows `PROC_THREAD_ATTRIBUTE_HANDLE_LIST` required for an exact HANDLE allowlist; Windows therefore retains explicit copy fallback or fail-closed required-direct behavior. A future audited Windows process adapter must preserve cancellation, exit observation, kill, and reap semantics before enabling that mode. Theoretical OS capability, implemented support, and configured deployment mode remain distinct.
