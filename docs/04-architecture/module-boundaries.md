...
GUI-3 instantiates `crates/desktop-contract` and `apps/estimator-desktop`. GUI-4 advances the contract to v2 and adapts the opaque selection token/native path authority to `DraftEstimateApplication` plus an explicitly configured `GeometryWorkerSupervisor`. The frontend owns only presentation/session display state. The Tauri host retains the raw selected path and complete `DraftEstimateSession` in native-only session state, then translates validated evidence into path-free provisional DTOs. It may not add formulas, rate selection, missing-state coercion, CAD parsing, or persistence behavior to the UI or host command layer.
