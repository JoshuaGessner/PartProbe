...
USE-2 adds `ShopSettingsApplication` for ordinary immutable draft persistence and a separate `ShopResourceCatalogApplication` for proposal-eligibility decisions. The catalog service verifies exact current settings/catalog/reviewed-selection versions, builds a fully validated immutable successor, evaluates a deployment policy, appends the content-minimized decision, and only then asks the repository to advance optimistically. `SqliteShopResourceCatalogAuthorizationAudit` implements only the audit port; it does not own policy or repository sequencing. The desktop composition root now opens independent host-owned settings-repository, catalog-repository, and audit connections and supplies `DesktopCatalogAuthorizationPolicy`. Ordinary startup uses its versioned deny-all rule. Its exact-profile/exact-operator mode is valid only when a trusted native identity/session boundary supplies the actor; it is not wired from environment or WebView input. Desktop contract v7 provides a path-free read-only catalog DTO and still does not call the activation service. The application contract owns transition sequencing; it does not supply authentication, role membership, a shop-reviewed allow configuration, desktop edit/activation authority, or estimate authority.
