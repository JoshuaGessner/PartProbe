...
The content-addressed native runtimes assembled for the evidenced Apple-Silicon and Ubuntu TASK-003 checkpoints are local engineering artifacts. The same manifest-bound shape is implemented for Windows x64, with canonical construction directories and app-local DLLs beside the worker. Run 31609710986 passes all native adapter/worker tests and assembles/re-verifies that Windows runtime shape, then stops at strict PE/import parsing before the configured host smoke. Header and dependency evidence now come from separate reviewed `dumpbin` modes; Windows PE closure and host success remain pending rerun. Separate run 31612819644 builds and executes an unsigned Debian-contained desktop shell without this native runtime; extraction is not installation, and its process survival is not a deployable analysis profile. These runtimes require an external explicit worker workspace and do not provide installation, a bundled MSVC runtime, signing, OS sandboxing, identity, authorization repositories, durable audit, backup, update, or controlled-data operations. Do not treat construction, local verification, or the shell-only Debian package as Standalone, Team/LAN, or controlled-data deployment acceptance.
