...
The content-addressed native runtimes assembled for the evidenced Apple-Silicon, Ubuntu-x86_64, and Windows-x64 TASK-003 checkpoints are local engineering artifacts. The Windows shape uses canonical construction directories and app-local DLLs beside the worker; run 31613259785 verifies 23 OCCT families across 24 x64 PE binaries and passes the configured host smoke. Separate run 31612819644 builds and executes an unsigned Debian-contained desktop shell without this native runtime; extraction is not installation, and its process survival is not a deployable analysis profile. These runtimes require an external explicit worker workspace and do not provide installation, a bundled MSVC runtime, signing, OS sandboxing, identity, authorization repositories, durable audit, backup, update, or controlled-data operations. Do not treat construction, local verification, or the shell-only Debian package as Standalone, Team/LAN, or controlled-data deployment acceptance.
