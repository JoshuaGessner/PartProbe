...
The current model/stock workspace keeps Estimate primary and opens through the **Model & stock** destination. It implements semantic Isometric/Front/Top/Right controls, actual-view feedback, an opaque selected model, and a textual inspector with representation, units, dimensions, warnings, confidence, analysis reference, and available proposal facts. Absent proposal facts read `Not available`; source mode does not draw stock. Future phases add the keyboard-selectable scene tree, Fit/edges/stock/reset controls, governed translucent stock, stock candidate editing, standard-size/availability state, and exact candidate pins. Enclosure failures require text as well as color. The viewer never implies workholding, fixture position, tool access, or CAM feasibility.
