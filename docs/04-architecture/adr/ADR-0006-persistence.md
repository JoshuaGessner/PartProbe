...
The additive `shop-resource-catalog-v1` type defines the future multi-entry and reviewed-selection domain boundary without changing stored bytes. SQLite remains schema v2; durable catalog activation requires a separately reviewed schema-v3 migration, replay from schema v2, application authorization, and a desktop-contract revision. This explicit deferral avoids silently reinterpreting existing `ShopResourceLibrary` snapshots.
