...
On 2026-09-15 all GPU-touching viewer operations gained terminal observed-loss guards, and a headless real Metal device-destruction test passed through the production callback. This narrows the lifecycle risk without establishing physical driver failure, native-surface/host fallback, full-device recreation, or approval of the proposed architecture. See [VIS-1 validation](../../06-quality/vis-1-renderer-validation.md).
