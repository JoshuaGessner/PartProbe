...
The supervisor now uses bounded stdio JSON, a cleared environment, atomically created private per-job directories, control-stream schema version 2, explicit cooperative-cancellation grace, acknowledgement validation, forced kill/reap fallback, and a consumed already-open source grant bound to the request capability. Asset-transport manifest schema version 2 binds the selected transport to job, correlation, capability, authorized length, SHA-256, and exactly one resource ID only for direct transport. `verified_private_copy` remains the default on every target. On Unix, the isolated platform adapter duplicates the granted file close-on-exec, allowlists only that descriptor plus stdio for `exec`, and the worker claims it once before independently rechecking regular-file type, manifest/request identity, exact length, quota, and hash into immutable bytes. Tests prove an intentionally inheritable unrelated descriptor does not survive, required direct creates no copy, malformed descriptor IDs fail closed, cancellation still works, and a removed pathname cannot replace the open grant. Windows `PreferDirect` still records its verified-copy fallback and `RequireDirect` still returns `ASSET_DIRECT_TRANSPORT_UNAVAILABLE`; exact HANDLE inheritance requires a later audited `PROC_THREAD_ATTRIBUTE_HANDLE_LIST` launcher. OCCT parses verified bytes through `ReadStream`, while the progress callback and forced-termination boundary remain. This evidence does not satisfy durable controlled-store persistence/retention, OS sandbox/no-network, descendant-process, memory/CPU, three-OS native, Windows HANDLE allowlisting, or packaging approval gates.
