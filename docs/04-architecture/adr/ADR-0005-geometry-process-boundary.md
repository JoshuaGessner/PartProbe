...
The supervisor now uses bounded stdio JSON, a cleared environment, atomically created private per-job directories, control-stream schema version 2, explicit cooperative-cancellation grace, acknowledgement validation, forced kill/reap fallback, and a consumed already-open source grant bound to the request capability. Asset-transport manifest schema version 1 binds the selected transport to job, correlation, capability, authorized length, and SHA-256. `verified_private_copy` is the only implemented transport; `PreferDirect` records why it fell back and `RequireDirect` returns `ASSET_DIRECT_TRANSPORT_UNAVAILABLE` before creating a workspace or launching. The worker independently opens the fixed private copy without following its final component, rechecks manifest/request identity, exact length, quota, and hash, and supplies immutable verified bytes to native adapter ABI version 3. OCCT parses those bytes through `ReadStream`, removing the verified-path second-open race, while the existing progress callback and forced-termination boundary remain. Tests cover manifest mismatch, unsupported direct transport, same-length hash tampering, length tampering, source deletion after grant, capability mismatch, cancellation/deadline acknowledgement, forced termination/reaping, and deterministic workspace cleanup. This evidence does not prove exclusion of arbitrary unrelated inheritable descriptors/HANDLEs, so direct transport remains unavailable. It also does not satisfy durable controlled-store persistence/retention, OS sandbox/no-network, descendant-process, memory/CPU, three-OS native, or packaging approval gates.
