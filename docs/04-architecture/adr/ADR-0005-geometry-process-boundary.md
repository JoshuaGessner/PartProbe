...
After those existing controls claim the authorized asset, the worker now dispatches ISO-10303-21 STEP, ZIP-framed 3MF, or STL comparison analysis from immutable bytes. STL/3MF success writes additive `geometry-mesh-snapshot-v1`; the supervisor claims and removes it through the existing controlled-output path, and the decoder binds its reference variant plus source hash. The old exact STEP result remains `geometry-snapshot-v1`. This adds no path-bearing IPC, no network capability, no persistence, and no broader sandbox claim.
