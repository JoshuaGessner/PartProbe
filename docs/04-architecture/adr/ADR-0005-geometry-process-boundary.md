...
The supervisor now uses bounded stdio JSON, a cleared environment, atomically created private per-job directories, a schema-versioned execute/cancel control stream, explicit cooperative-cancellation grace, acknowledgement validation, forced kill/reap fallback, a consumed already-open source grant bound to the request capability, fixed-name source staging, authorized-length and SHA-256 checks, read-only staged bytes, and cleanup. User cancellation and wall deadline remain distinct; unacknowledged cancellation responses fail protocol validation, while grace expiry produces distinct force-termination diagnostics. It does not reopen a source pathname, and tests prove path removal after grant creation, capability mismatch, source-length drift, cooperative acknowledgement, deadline cancellation, uncooperative termination, reaping, and workspace cleanup are handled deterministically. Native adapter ABI version 2 wires the worker cancellation state into OCCT's progress indicator for STEP root transfer, contains callback panics at the C ABI, and returns a sanitized cancellation code; checks surround native phases that do not accept progress. A cross-platform low-level opener rejects a linked final component while opening read-only: Unix uses `O_NOFOLLOW`, while Windows opens and rejects a reparse point with identification-only security quality of service. Worker output referenced by a validated response is reopened with that policy, bounded and copied into immutable supervisor-owned bytes, hashed, bound to the opaque reference, and unlinked before the job directory is removed; unreferenced output is discarded. This evidence narrows the design but does not make OCCT file parsing or property computation internally interruptible and does not satisfy direct worker handle passing, durable controlled-store persistence/retention, OS sandbox/no-network, descendant-process, memory/CPU, or packaging approval gates.
