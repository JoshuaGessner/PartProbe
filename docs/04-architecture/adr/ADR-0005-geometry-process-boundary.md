...
After those existing controls claim the authorized asset, the worker dispatches ISO-10303-21 STEP, ZIP-framed 3MF, or STL comparison analysis from immutable bytes. STL/3MF success writes additive `geometry-mesh-snapshot-v1`; ABI-v4 STEP writes `geometry-step-analysis-v1`, embedding the complete unchanged legacy `geometry-snapshot-v1` plus a same-source AABB derivative. The supervisor claims and removes either output through the existing controlled-output path, and the decoder binds its reference variant plus source hash. Legacy exact output remains accepted with explicit envelope absence. This adds no path-bearing IPC, no network capability, no persistence, and no broader sandbox claim.

VIS-2 defines a separate deterministic display-artifact framing v1 over the existing `ControlledWorkerOutput` abstraction. One bounded binary artifact carries exact `PPDSCENE` magic, schema/flags, bounded manifest and chunk counts, then ordered length-delimited native display bytes. The decoder rejects the wrong controlled reference, header/version/flag/manifest failures, truncation, trailing bytes, oversized spans, stale source/analysis identity, altered chunk hashes/lengths, non-finite positions, and out-of-range indices before returning nonserializable arrays. Worker request/response schema v2 preserves schema-v1 analysis behavior and adds one exact optional display request plus a separately typed response reference; unknown versions, unsolicited output, silent omission, and contradictory availability evidence fail closed. The supervisor claims `partprobe-output.json` and `partprobe-display-scene.bin` under the same aggregate quota through the existing no-follow/read/hash/remove boundary and drops every partial claim on failure. The typed application service can retain only a source/analysis-bound decoded scene. Native worker emission and renderer handoff remain absent; the real worker currently preserves analysis and reports `DISPLAY_SCENE_UNAVAILABLE` for a display request. Display output never replaces or relabels the authoritative analysis output.
