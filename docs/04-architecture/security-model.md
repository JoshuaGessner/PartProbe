...
No role catalog, project-membership repository, classification policy, identity provider, durable audit store, deployment-specific allow policy, network/filesystem sandbox or hard filesystem quota, hard macOS memory control, or hostile Unix descendant containment is implemented; those remain required adapters and approval evidence rather than inferred defaults. Aggregate job output is now checked and cleaned at supervisor poll/post-exit boundaries, but that evidence cannot be promoted to an OS-enforced quota.
