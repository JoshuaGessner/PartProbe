...
The headless `security` and `application` crates define versioned allow/deny decisions, explicit deny-all baselines, content-minimized authorization contexts, append-only audit ports, and fail-closed application services. The local geometry-asset read service requires decision audit before resolving a capability. USE-2 adds a separate catalog proposal-activation service that verifies exact current settings/catalog/reviewed-selection versions, requires policy-decision audit before mutation, and creates new immutable decision versions; denial or audit failure leaves state unchanged. Schema v4 implements that catalog audit port in SQLite with exact-version foreign keys, append-only triggers, normalized event hashes, exact idempotent correlation retries, and changed-correlation rejection. Native Settings composes independent repository/audit connections with `DesktopCatalogAuthorizationPolicy`. Ordinary startup uses its versioned deny-all rule; an exact-profile/exact-operator rule is available only for a trusted native identity/session source and has controlled mismatch/allow/reopen evidence. No environment shortcut, UI confirmation, or WebView actor enables it. An event records the decision before mutation and does not imply the later operation succeeded. Desktop contract v7 exposes the persisted catalog only through typed path-free read-only snapshots; its save request carries no catalog mutation, and the host rejects legacy save for catalog-backed revisions. The open directory capability is bound to a stable root ID, so geometry policy evaluation and filesystem containment use the same root identity. The narrow `platform` crate implements Unix descriptor and Windows HANDLE duplication, exact child-resource allowlisting, one-shot worker claim, the standard/direct child lifecycle, the Linux parser-phase syscall filter, and the partial resource controls listed above behind focused unsafe review; this is not an OS sandbox.
