...
2. **Import worker:** separate, least-privilege process for geometry/document parsing and thumbnail/derivative creation; receives either one explicitly allowlisted inherited resource or an explicitly selected, identity/hash/length-bound verified private copy. Unix descriptor inheritance allowlists stdio plus the intended source at `exec`; Windows direct launch uses `PROC_THREAD_ATTRIBUTE_HANDLE_LIST` to allowlist child stdin/stdout, null stderr, and the intended source HANDLE. Each platform proves an intentionally inheritable unrelated resource is absent in the child. Current partial resource containment covers Unix CPU/file/core/process-group controls, Linux address-space limits, a suspended Windows Job with CPU/committed-memory/one-process/tree-kill controls, and portable poll-bounded private-workspace aggregate-output monitoring plus cleanup. Linux `x86_64`/`aarch64` additionally applies a thread-synchronized parser-phase seccomp filter after the authorized asset is immutable, denying socket, `io_uring`, process/thread creation, and executable replacement syscalls; configured Ubuntu x86_64 OCCT evidence passes under that boundary. macOS/Windows parser egress denial, general filesystem sandboxing and hard storage quotas, hard macOS memory, hostile macOS descendant escape, Linux-aarch64 native compatibility, and complete three-OS containment remain unimplemented, so “no direct access outside approved resources” remains a target control rather than a completed sandbox claim.
...
The headless `security` and `application` crates define versioned allow/deny decisions, explicit deny-all baselines, content-minimized authorization contexts, append-only audit ports, and fail-closed application services. The local geometry-asset read service requires decision audit before resolving a capability. USE-2 adds separate catalog draft and proposal-activation services; the latter verifies exact current versions, requires policy-decision audit before mutation, and creates new immutable decision versions. Schema v4 implements that audit port in SQLite. Native Settings composes independent repository/audit connections. Contract v8's path-free activation request carries no actor, profile, time, correlation, policy, storage, or path authority. Contract v9's complete bounded draft request carries no selection, actor, profile, time, policy, storage, or path authority. The host derives the trusted facts, enforces record ceilings, reconstructs typed values, and rejects immutable-version reuse. Ordinary startup has no authenticated actor, both catalog calls return native-identity unavailability, activation remains deny-all, and the WebView invokes neither. No environment shortcut or UI confirmation enables identity or policy. An event records an activation decision before mutation and does not imply the later operation succeeded. Desktop contract v7 continues to expose persisted catalogs through typed path-free snapshots; its legacy save request carries no catalog mutation. The open directory capability is bound to a stable root ID, so geometry policy evaluation and filesystem containment use the same root identity. The narrow `platform` crate implements Unix descriptor and Windows HANDLE duplication, exact child-resource allowlisting, one-shot worker claim, the standard/direct child lifecycle, the Linux parser-phase syscall filter, and the partial resource controls listed above behind focused unsafe review; this is not an OS sandbox.
...
No role catalog, project-membership repository, classification policy, identity provider, or shop-reviewed shipped catalog allow configuration is implemented; those remain required adapters and approval evidence rather than inferred defaults. The exact-operator policy alone does not authenticate its actor. General filesystem sandboxing/hard quotas, hard macOS memory control, macOS/Windows parser egress denial, and hostile macOS descendant containment also remain absent. The schema-v4 catalog decision store is local hash/trigger/foreign-key evidence only; it has no separate access-control layer, external signature/anchor, general security-event coverage, or release-approved retention/export policy. The catalog service's actor/profile/version context, deny-all runtime, and controlled exact-operator test are not SEC-003 completion. Linux syscall-denial fixtures cover `x86_64` and `aarch64`; configured native OCCT compatibility is evidenced only on Ubuntu x86_64, and neither result is packet-level policy or a general network/filesystem sandbox. Aggregate job output is checked and cleaned at supervisor poll/post-exit boundaries, but that evidence cannot be promoted to an OS-enforced quota.
