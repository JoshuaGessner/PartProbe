...
`shop-resource-catalog-v1` is currently an additive domain-only contract. It is intentionally absent from `ShopSettingsDraft`, SQLite schema v2, and desktop contract v6. The migration decision for this slice is no database rewrite: durable catalog snapshots, selection decisions, and current-pointer semantics require an explicit schema-v3 migration with schema-v2 replay fixtures, plus a reviewed application authorization and desktop-contract revision. The existing starter bundle remains the only persisted resource shape until that gate passes.
