...
GUI-3 instantiated the outer two layers; GUI-4 now connects the native host to `DraftEstimateApplication` and a separately configured `GeometryWorkerSupervisor` without claiming repositories, a viewer, or product support. `apps/estimator-desktop` owns the Leptos CSR presentation; its `src-tauri` package owns the native source path, selected extension hint, application adapter, in-memory authorization audit, worker configuration, active cancellation token, and retained draft session; and `crates/desktop-contract` owns the versioned framework-neutral command/event DTO. The WebView submits opaque native session identifiers plus explicit text/confirmation inputs and receives only path-free provisional facts and result traces. Contract v4 keeps v3's exact five commands and one event, but the analysis result is now a tagged exact-B-rep/mesh union. The native adapter content-minimizes validated mesh evidence into decimal text, optional measurements, format/unit basis, categorical confidence, topology/welding state, warnings, versions, and hashes; full parser-owned provenance remains in native application state.
