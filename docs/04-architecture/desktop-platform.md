...
GUI-3 instantiated the outer two layers; GUI-4 connects the native host to `DraftEstimateApplication` and a separately configured `GeometryWorkerSupervisor` without claiming a viewer or product support. USE-2 now connects the host to `ShopSettingsApplication` and its SQLite repository for bounded durable rate/pricing plus optional starter-resource drafts. `apps/estimator-desktop` owns the Leptos CSR presentation; its `src-tauri` package owns native source/database paths, selected extension hint, application adapters, in-memory analysis authorization audit, worker configuration, active cancellation token, and retained draft session; and `crates/desktop-contract` owns the versioned framework-neutral command/event DTO. The WebView submits opaque native session identifiers plus explicit text/confirmation inputs and receives only path-free provisional facts, result traces, and Settings draft values. Contract v5 preserves v4's five analysis/estimate commands and one event and adds exactly two Settings load/save commands; contract v6 keeps that exact command/event surface and adds only optional material/offer/stock/machine/runtime Settings fields. The native adapter content-minimizes validated mesh evidence into decimal text, optional measurements, format/unit basis, categorical confidence, topology/welding state, warnings, versions, and hashes; full parser-owned provenance remains in native application state.
