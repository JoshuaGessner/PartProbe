...
GUI-3 instantiated the outer two layers; GUI-4 connects the native host to `DraftEstimateApplication` and a separately configured `GeometryWorkerSupervisor` without claiming a viewer or product support. USE-2 now connects the host to `ShopSettingsApplication` and its SQLite repository for bounded durable rate/pricing plus optional starter-resource/catalog drafts. `apps/estimator-desktop` owns the Leptos CSR presentation; its `src-tauri` package owns native source/database paths, selected extension hint, application adapters, in-memory analysis authorization audit, worker configuration, active cancellation token, and retained draft session; and `crates/desktop-contract` owns the versioned framework-neutral command/event DTO. The WebView submits opaque native session identifiers plus explicit text/confirmation inputs and receives only path-free provisional facts, result traces, and Settings draft values. Contract v5 preserves v4's five analysis/estimate commands and one event and adds exactly two Settings load/save commands; v6 adds starter resource fields and v7 adds complete read-only catalog snapshots while retaining that exact command/event surface. Contract v8 adds one activation command and v9 adds one catalog-draft save command, bringing the exact surface to nine. The legacy save request still cannot mutate catalogs. The native adapter content-minimizes validated mesh evidence into decimal text, optional measurements, format/unit basis, categorical confidence, topology/welding state, warnings, versions, and hashes; full parser-owned provenance remains in native application state.

Mesh evidence is reviewable but not estimate-authoritative. STL measurements remain labeled as unresolved source coordinates; 3MF may carry canonical-millimetre measurements, but neither mesh variant may enter the deterministic calculation path. `DraftEstimateSession::evaluate` returns `Unavailable` for mesh before review, manual inputs, rates, or pricing can authorize an estimate, and the frontend withholds the estimate form as a secondary UX control. Exact STEP continues to use the existing `geometry-snapshot-v1` and calculation rules. The selected filename extension is only a host-owned intake hint; a mismatch with content-derived exact/STL/3MF analysis fails visibly. No path, bytes, parser authority, component graph, or metadata content crosses the bridge.
...
The desktop adapter is intentionally fail closed. `PARTPROBE_GEOMETRY_WORKSPACE` identifies the external controlled workspace. An explicit developer `PARTPROBE_NATIVE_RUNTIME` may identify a separately assembled runtime root; otherwise a packaged host checks exactly the `partprobe-native-runtime` child of Tauri's resolved resource directory. It does not search adjacent directories or accept independent worker/library overrides. Before constructing the supervisor, the native host validates the schema-v1 runtime manifest, exact pinned source/build/host provenance, every regular-file size/hash, safe local symlink, executable bit, direct-library fingerprint, complete manifest closure, and absence of extra or special artifacts. Only that verified manifest may supply the worker and native-library paths. The async Tauri commands move synchronous application/worker/database execution off the UI task. Analysis applies a narrow local-developer-session policy, appends the decision to a session-memory audit, and commits the returned `DraftEstimateSession` only if the same selection token remains current. A selection-bound native cancellation token reaches the worker supervisor; the lightweight cancel command is idempotent and cannot signal another selection. The estimate command validates the retained analysis token, constructs all typed review/input/rate/pricing values before mutating the session, and invokes only `DraftEstimateSession::evaluate`. The host resolves Settings storage only beneath Tauri's platform application-data directory, owns database creation/opening, and calls `ShopSettingsApplication`; the WebView cannot choose or open a database. Native Settings composition opens independent catalog-draft, catalog-activation, and schema-v4 audit connections. Contracts v8/v9 add path-free activation and complete bounded draft-save requests; actor/profile/time/correlation authority remains host-owned, and neither request accepts selections or storage/path authority outside its governed purpose. Ordinary startup supplies no actor, returns explicit unavailability for both catalog commands, and retains deny-all activation; the WebView invokes neither. No environment allow switch exists. Resource input is converted into validated domain types with explicit units, source, draft lifecycle, currency, and exact internal references before storage. The draft adapter enforces per-kind ceilings before typed mapping, preserves matching immutable source evidence across equivalent decimal text normalization, and rejects changed content under a reused version. Missing/invalid configuration, verification failure, stale tokens/revisions, incomplete confirmation, invalid text, unavailable rates/storage, or immutable-version conflicts return bounded typed states. The content-addressed manifest detects the checked changes at startup but is neither a signature nor a control against post-verification mutation. Linux run 31624958771 proves the package-aware resource path through an unsigned extracted Debian payload, repeated verification, real retained-session analysis, and bounded headless launch. Durable production identity/rate/resource approval, protected-storage policy, interactive cross-platform acceptance, signing, and production deployment policy remain open.
...
GUI-3 provides the shell evidence. GUI-4 adds the original exact five-command manifest/capability tests, token-only async analysis and cancellation regressions, fail-closed configuration, post-authorization adapter/session evidence, path-redacted provisional/result DTOs, explicit unavailable/blocked/cancelled/error UI states, deterministic exact-result reconciliation, strict native/WASM lint, and offline frontend bundling. Contract-v4 tests add exact/mesh tagged round trips, STL/3MF extension intake, format-match enforcement, preserved optional mesh measurements, unknown-unit labels, native/static-adapter mesh mapping, and application-enforced estimate unavailability even for a complete forged request. Contract-v5 tests require the expanded exact seven-command manifest/capability/handler, path-free Settings DTOs, first-run absence, USD save, stale-writer rejection, reopen, and reset calculation confirmations. Contract-v8 tests require the exact eighth activation permission/handler, path-free authority-minimized request, native evidence derivation, unavailable/denied no-mutation behavior, and no WebView invocation. Contract-v9 tests require the exact ninth draft-save permission/handler, complete bounded path-free request without selection or host authority, native identity/time mapping, save/reopen and active-selection downgrade, immutable-version and record-limit rejection, and no WebView invocation. GUI-5 adds one configured Apple-Silicon real-worker STEP host smoke plus actual-app keyboard, semantic, cancellation, failure/recovery, and review-reset evidence. The follow-on TASK-003 checkpoint adds portable Rust runtime-verifier unit coverage and repeats the real STEP host smoke with only the verified runtime root and external workspace configured. Configured real-worker desktop mesh smokes, Settings catalog-editor/accessibility and three-OS package evidence, screen-reader/full visual matrix, signed-package, viewer, PDF, estimate persistence, and update evidence remain open.
