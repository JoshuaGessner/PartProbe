...
GUI-3 instantiated the outer two layers; GUI-4 connects the native host to `DraftEstimateApplication` and a separately configured `GeometryWorkerSupervisor` without claiming a viewer or product support. USE-2 connects the host to `ShopSettingsApplication` and SQLite. `apps/estimator-desktop` owns presentation; `src-tauri` owns paths, database, native/application adapters, worker/cancellation, and retained sessions; `crates/desktop-contract` owns framework-neutral DTOs. Contract v8 added catalog activation, v9 added catalog-draft save, and v10 adds exactly one proposal command for a total of ten. Contract v10 also adds path-free exact STEP AABB, starter-Draft proposal, explicit adoption, and result-exclusion DTOs. The WebView cannot calculate or alter proposal formulas; the host reloads the exact settings revision and application recomputes from retained geometry before adoption. Legacy manual estimate and Settings DTOs remain accepted.
...
The first slice needs a responsive model viewer but must not bind the domain to a renderer. A `model-viewer` crate exposes scene, camera, selection, and render-result contracts. Contract v11's developer adapter exposes a native `wgpu` surface beside a bounded child WebView inside the existing PartProbe window; it sends no geometry into the WebView and creates no popup. Rendering, feature overlays, and picking remain visualizations of immutable analysis snapshots, and the current scene is synthetic until VIS-2 supplies the governed derivative.
...
GUI-3–GUI-5 provide the shell, typed native analysis/estimate, and configured worker evidence. Contract-v8/v9 tests cover exact catalog command permissions and authority minimization. Contract v11 preserves v10's path-free AABB/proposal/adoption DTOs and adds one exact path-free viewer workspace command, requiring eleven-command manifest/permission/capability/handler parity. Application tests prove model-sensitive cube/prism proposal and adoption behavior; VIS-1 tests prove only a synthetic same-window viewport. Packaged UI adoption and broader platform evidence remain immediate live-test gates. Settings catalog-editor/accessibility, signed packages, source-bound viewer data, PDF, estimate persistence, and update evidence remain open.
