...
GUI-3 instantiated the outer two layers; GUI-4 now connects the native host to `DraftEstimateApplication` and a separately configured `GeometryWorkerSupervisor` without claiming repositories, a viewer, or product support. `apps/estimator-desktop` owns the Leptos CSR presentation; its `src-tauri` package owns the native source path, application adapter, in-memory authorization audit, worker configuration, and retained draft session; and `crates/desktop-contract` owns the versioned framework-neutral command/event DTO. The WebView submits only the opaque selection token and receives path-free provisional facts and explicit estimate unavailability.
...
The current developer adapter is intentionally fail-closed configuration, not packaging. `PARTPROBE_GEOMETRY_WORKER`, `PARTPROBE_GEOMETRY_WORKSPACE`, and `PARTPROBE_OCCT_ROOT` must identify an existing executable, controlled workspace, and native-library root. The async Tauri command moves synchronous application/worker execution off the UI task, applies a narrow local-developer-session policy, appends the decision to a session-memory audit, and commits the returned `DraftEstimateSession` only if the same selection token remains current. Missing configuration, stale tokens, or failures return bounded diagnostic IDs and retain the selected source for retry. Durable identity/audit, cancellation, packaging, and production deployment policy remain open.
...
GUI-3 provides the shell evidence. GUI-4 adds exact three-command manifest/capability tests, token-only async analysis-source regression, fail-closed configuration, post-authorization adapter/session evidence, path-redacted provisional DTOs, explicit unavailable/error UI states, strict native/WASM lint, offline frontend bundling, and a responsive semantic/visual preview. It does not satisfy configured native end-to-end, cancellation, complete estimate workflow, screen-reader, three-OS desktop launch, signed-package, viewer, PDF, or update evidence.
