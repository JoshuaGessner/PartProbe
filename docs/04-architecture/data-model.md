...
The next additive in-memory contract, `shop-resource-catalog-v1`, defines bounded collections for each starter-record kind plus exact reviewed selection decisions. It rejects duplicate identity/version pairs, dangling offer/runtime/selection references, mismatched selected material/machine chains, unreviewed active children, and more than one active proposal selection. Selection decisions retain actor/time/reason and distinguish reviewed, active-for-proposals, and retired states. This type is not referenced by persisted `ShopSettingsDraft`, so SQLite remains schema v2 and desktop remains contract v6; the migration decision is to require a later schema-v3/contract revision before durable or UI activation.
