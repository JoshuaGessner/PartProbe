...
The `shop-resource-catalog-v1` contract defines bounded collections for each starter-record kind plus exact reviewed selection decisions. It rejects duplicate identity/version pairs, dangling offer/runtime/selection references, mismatched selected material/machine chains, unreviewed active children, and more than one active proposal selection. Selection decisions retain actor/time/reason and distinguish reviewed, active-for-proposals, and retired states. Schema v3 adds the catalog as a mutually exclusive optional `ShopSettingsDraft` resource context and persists immutable aggregate, child, selection, and settings-reference evidence without rewriting schema-v1/v2 payloads. `ShopResourceCatalogDraftApplication` governs immutable catalog draft creation/editing: exact current expectations are required, existing records cannot disappear or change bytes, new identities begin at Draft version one, successors use the next Draft version, and selections remain application-owned. Editing an active catalog newly versions that decision back to Reviewed before save. The separate `ShopResourceCatalogApplication` turns one exact reviewed selection into a newly versioned active-for-proposals decision only after a policy decision is appended; switching selections also creates a new reviewed version of the prior active decision. Schema v4 persists the separate content-minimized authorization decision—exact prior versions, actor/time, operation, correlation, policy, outcome, and reason—without treating it as operation-success or resource-value evidence. Desktop contract v7 maps the complete catalog into typed path-free read-only Settings snapshots. Contract v8 carries only exact activation expectations plus reason; contract v9 carries exact draft expectations, complete bounded five-kind contents, and reason, but no selections or host identity/time/profile/path/storage authority. The native adapter reconstructs typed values, preserves matching immutable source evidence, and rejects changed bytes under reused versions. Ordinary startup remains identity-unavailable and the WebView invokes neither catalog command. Authenticated identity/roles, a shop-reviewed shipped allow configuration, and governed desktop catalog editing/activation UI remain required.
