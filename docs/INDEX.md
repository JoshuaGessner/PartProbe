...
- **Next actions:** Review full native OCCT package run 32607742609 when it finishes; do not wait live. Continue TASK-004 with 3MF comparison, additional binary/malformed cases, self-intersection/confidence policy, and worker/desktop integration without promoting the synthetic STL slices to support. Obtain GUI-1 fixture review; add interactive Windows evidence, Windows/macOS package integration, macOS/Windows parser egress denial, general filesystem containment, target dynamic-link/license/SBOM review, and signed application integration. Full TASK-005 durable desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
...
- **Geometry:** Intake/units/hash/healing/validity/basic measurements/stock/feature/access/setup stages are specified. Three STL, one OCCT-generated analytic STEP, one manually authored analytic STEP, and one invalid-entity STEP fixture exist. The optional local worker produces provisional exact-B-rep measurements through OCCT; separate bounded ASCII/binary STL comparison analyzers reproduce the three mesh fixtures while keeping source units unresolved and open-mesh volume unavailable. Developer runtimes can be assembled and content-verified independently of Cargo output on evidenced Apple-Silicon, Ubuntu-x86_64, and Windows-x64 configurations. 3MF, reviewed corpus accuracy, sandboxing, signed three-OS packaging, and product support remain absent.
...
| STL/3MF mesh import/validation | In Progress: bounded ASCII/binary STL fixture evidence passes; 3MF, broader binary cases, confidence/self-intersection policy, desktop integration, and representative corpus remain open | ADR-0004 and TASK-004 |
