...
- **Next actions:** Continue TASK-004 with union, item/vendor-metadata, and additional archive-layout cases, then additional binary/malformed cases, self-intersection/confidence policy, and worker/desktop integration without promoting the synthetic slices to support. Obtain GUI-1 fixture review; add interactive Windows evidence, Windows/macOS package integration, macOS/Windows parser egress denial, general filesystem containment, target dynamic-link/license/SBOM review, and signed application integration. Full TASK-005 durable desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
...
- **Geometry:** Intake/units/hash/healing/validity/basic measurements/stock/feature/access/setup stages are specified. Three STL, fifteen 3MF, one OCCT-generated analytic STEP, one manually authored analytic STEP, and one invalid-entity STEP fixture exist. The optional local worker produces provisional exact-B-rep measurements through OCCT; bounded ASCII/binary STL plus 3MF comparison analyzers reproduce thirteen successful mesh fixtures and exact rejections for five adversarial 3MF packages. Success evidence covers all six Core units/default, direct through two-link linear component transforms, and bounded model-metadata presence converted to canonical millimetres; failure evidence covers selected branching/reference/object-metadata/package threats without geometry authority. The corpus remains derived from the same analytic cube rather than independent representative geometry. Developer runtimes can be assembled and content-verified independently of Cargo output on evidenced Apple-Silicon, Ubuntu-x86_64, and Windows-x64 configurations. Union/item/vendor-metadata coverage, reviewed corpus accuracy, sandboxing, signed three-OS packaging, and product support remain absent.
...
| STL/3MF mesh import/validation | In Progress: bounded ASCII/binary STL plus ten successful direct/bounded-linear-component-chain 3MF package/unit/transform/model-metadata fixtures and five typed adversarial rejection fixtures pass; every Core unit/default plus selected branching/reference/object-metadata/package failures are persisted, while unions, item/vendor metadata, further binary/adversarial cases, confidence/self-intersection policy, worker/desktop integration, and representative corpus remain open | ADR-0004 and TASK-004 |
