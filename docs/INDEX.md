...
| Isolated geometry-worker IPC/crash/resource boundary | Bounded subprocess, consumed already-open grant, identity-bound manifest v2, explicit verified-copy transport, exact Unix descriptor and Windows HANDLE allowlisting with unrelated-resource exclusion, worker-side type/hash/length/quota verification, final-component-safe opening, private workspaces, immutable output claiming, governed storage handoff, cooperative/forced cancellation, Unix CPU/file/process-group controls, Linux memory limits, and suspended Windows Job CPU/memory/process containment pass in the 99-test macOS/100-test Linux+Windows baseline. OCCT verified-byte-stream parsing and transfer cancellation pass locally through adapter ABI v3; representative mid-transfer proof, internally uninterruptible phase bounds, network/filesystem sandboxing, hard macOS memory, hostile Unix descendant, aggregate-output, packaging, and native three-OS evidence remain pending | ADR-0005 and TASK-003 |
