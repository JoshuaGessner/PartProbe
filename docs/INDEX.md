...
- **Recently completed:** TASK-004 adds deterministic public `FIX-MESH-034`–`038`: malformed-model-XML, forbidden-document-type, archive-entry-count, non-finite-binary-normal, and binary-triangle-quota rejection inputs. Typed failure expectations pin each source hash and exact sanitized diagnostic while forbidding geometry evidence. The accepted boundaries and diagnostics are unchanged, so 3MF parser v5 and binary STL parser v1 remain pinned and no migration applies. Rust CI run 32737358886 for the preceding `FIX-MESH-029`–`033` checkpoint passes macOS and Ubuntu but fails Windows tooling because Python normalized a backslash ZIP name on that host; this checkpoint constructs the portable slash form first and rewrites equal-length raw local/central filename fields while preserving the governed hash. Three-OS run 32734198822 remains the last fully passing default matrix. This is parser-level rejection evidence, not installation, worker/desktop integration, full accessibility/usability, supported importer, signing, legal approval, or distribution approval.
