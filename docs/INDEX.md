...
- **Overall status:** Pre-alpha engineering foundation. TASK-001 and TASK-002 calculation/rate mechanics pass local and three-OS CI; TASK-003 has cross-platform worker contracts, partial OS containment including poll-bounded aggregate private-workspace output monitoring and Linux `x86_64`/`aarch64` parser-phase socket/process syscall denial, Apple Silicon provisional OCCT byte-stream measurements, reproducible pinned local construction, one manually authored analytic STEP fixture, and a separately assembled developer runtime containing the worker plus complete observed OCCT closure that the desktop verifies before launch. GUI-2 through GUI-5 establish one bounded Apple-Silicon internal developer path from native selection and real OCCT analysis through revision-bound review, complete inputs, cancellation/recovery, and deterministic USD 702 trace. Durable repositories, Windows/Linux native construction, signed/distributable packaging, a supported production importer, calibrated deployment, and a release package do not exist
- **Recently completed:** Supported Linux workers now prepare `no_new_privs` before creating their cancellation reader and install a thread-synchronized seccomp filter after the authorized source is immutable. Feature-off subprocess evidence denies socket and descendant creation with sanitized failures. The prior portable aggregate workspace-output monitor and cleanup remain intact. These are narrow Linux syscall and portable supervisor controls, not a general filesystem sandbox, hard disk quota, or configured Linux OCCT proof. GUI-5 and GUI-4 remain complete only for their bounded internal checkpoints; GUI-1 implementation remains ready for formal fixture/security review.
...
- **Next actions:** Obtain GUI-1 fixture review; verify the Linux parser filter with the pinned native OCCT worker; port the pinned OCCT construction plus schema-v1 runtime assembly/in-host-verification/launch evidence to Windows and Linux; add macOS/Windows parser egress denial, general filesystem containment, target dynamic-link/license/SBOM review, and signed application integration while closing remaining TASK-003 containment. TASK-004 mesh import, full TASK-005 durable desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
...
- **Geometry:** Intake/units/hash/healing/validity/basic measurements/stock/feature/access/setup stages are specified. Two STL, one OCCT-generated analytic STEP, one manually authored analytic STEP, and one invalid-entity STEP fixture exist. The optional local worker produces provisional exact-B-rep measurements through OCCT; its Apple-Silicon developer runtime can now be assembled and content-verified independently of Cargo output. Reviewed corpus accuracy, sandboxing, signed three-OS packaging, and product support remain absent.
...
| OCCT STEP import/measurement/healing/package corpus | Exact 8.0.0 Apple Silicon automated construction plus supervised generated-cube and manually authored-prism measurements pass; fixture review, broader independent corpus, three-OS artifacts, legal/package evidence pending | ADR-0002 and TASK-003 |
| Isolated geometry-worker IPC/crash/resource boundary | Bounded subprocess, consumed already-open grant, identity-bound manifest v2, exact resource allowlisting, worker byte verification, controlled workspaces/output, cooperative/forced cancellation, poll-bounded aggregate workspace-output monitoring, Linux parser socket/process syscall denial, and partial Unix/Linux/Windows resource controls pass. The validated provisional snapshot binds claimed bytes to the source. Local tooling constructs and fingerprints the pinned Apple Silicon OCCT root before strict native checks, and generated plus manually authored analytic STEP evidence passes through adapter ABI v3 and the supervisor. Fixture review, representative mid-transfer proof, internally uninterruptible phase bounds, macOS/Windows parser egress denial, general filesystem sandboxing and hard filesystem quotas, hard macOS memory, hostile macOS descendants, packaging, configured native Linux proof, and native Windows/Linux evidence remain pending | ADR-0005 and TASK-003 |
