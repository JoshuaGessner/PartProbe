...
- **Overall status:** Pre-alpha engineering foundation. TASK-001 and TASK-002 calculation/rate mechanics pass local and three-OS CI; TASK-003 has cross-platform worker contracts, partial OS containment including poll-bounded aggregate private-workspace output monitoring, Apple Silicon provisional OCCT byte-stream measurements, reproducible pinned local construction, one manually authored analytic STEP fixture, and a separately assembled developer runtime containing the worker plus complete observed OCCT closure that the desktop verifies before launch. GUI-2 through GUI-5 establish one bounded Apple-Silicon internal developer path from native selection and real OCCT analysis through revision-bound review, complete inputs, cancellation/recovery, and deterministic USD 702 trace. Durable repositories, Windows/Linux native construction, signed/distributable packaging, a supported production importer, calibrated deployment, and a release package do not exist
- **Recently completed:** The geometry supervisor now polls its owned private job directory, preserves the fixed staged-input invariant, bounds all other regular-file bytes and entry count, rejects special/nested entries, rechecks after worker exit, force-terminates violations, and recursively removes worker scratch files. A hostile cross-platform fixture exceeds the aggregate budget with two individually sub-limit files and is terminated with a sanitized diagnostic. This is supervisory evidence, not an OS-enforced disk quota or filesystem sandbox. GUI-5 and GUI-4 remain complete only for their bounded internal checkpoints; GUI-1 implementation remains ready for formal fixture/security review.
...
- **Next actions:** Obtain GUI-1 fixture review; port the pinned OCCT construction plus schema-v1 runtime assembly/in-host-verification/launch evidence to Windows and Linux; add target dynamic-link/license/SBOM review and signed application integration while closing remaining TASK-003 containment. TASK-004 mesh import, full TASK-005 durable desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
...
| Isolated geometry-worker IPC/crash/resource boundary | Bounded subprocess, consumed already-open grant, identity-bound manifest v2, exact resource allowlisting, worker byte verification, controlled workspaces/output, cooperative/forced cancellation, poll-bounded aggregate workspace-output monitoring, and partial Unix/Linux/Windows resource controls pass. The validated provisional snapshot binds claimed bytes to the source. Local tooling constructs and fingerprints the pinned Apple Silicon OCCT root before strict native checks, and generated plus manually authored analytic STEP evidence passes through adapter ABI v3 and the supervisor. Fixture review, representative mid-transfer proof, internally uninterruptible phase bounds, network/filesystem sandboxing and hard filesystem quotas, hard macOS memory, hostile Unix descendant, packaging, and native Windows/Linux evidence remain pending | ADR-0005 and TASK-003 |
