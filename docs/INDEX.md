...
- **Recently completed:** `crates/native-runtime` now repeats the schema-v1 source/build/host/artifact/closure verification inside the desktop host, supplies the only allowed worker/library paths, and maps failure to a sanitized unavailable state. Five verifier regressions and the opt-in real-host STEP-to-estimate smoke pass against the dedicated 49 MiB runtime using only its root plus an external workspace. GUI-5 and GUI-4 remain complete only for their bounded internal checkpoints; GUI-1 implementation remains ready for formal fixture/security review.
