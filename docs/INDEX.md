...
- **Overall status:** Pre-alpha engineering foundation. TASK-001 and TASK-002 calculation/rate mechanics pass local and three-OS CI; TASK-003 has cross-platform worker contracts, partial OS containment including poll-bounded aggregate private-workspace output monitoring and Linux `x86_64`/`aarch64` parser-phase socket/process syscall denial, provisional OCCT byte-stream measurements, one manually authored analytic STEP fixture, and host-verified developer-runtime evidence on Apple Silicon, Ubuntu x86_64, and Windows x64. TASK-004 now has bounded ASCII/binary STL and direct/linear-component-chain 3MF comparison slices for nineteen successful governed synthetic mesh fixtures plus twenty-nine persisted rejected mesh inputs. Versioned exact-predicate intersection evidence and categorical reason-coded confidence cap validated mesh at `Low`; unresolved units, topology defects, detected intersection, or indeterminate coplanar overlap require `NeedsReview`, and unauthorized closed-measurement states withhold volume/centroid. Persisted STL evidence now isolates open boundary, reversed-facet winding, non-manifold shared-edge topology, detected intersection, and selected rejection boundaries. The thirty-four 3MF packages cover every Core length-unit declaration plus the normative millimetre default, direct through two-link linear transform/provenance cases, bounded well-known model metadata presence without retained content, three isolated legal OPC variations, and exact fail-closed graph/structural/metadata/extension/archive/XML/resource diagnostics. Supported general DAG/material semantics, broader representative/exporter coverage, production mesh tolerance, worker/desktop integration, and support remain open. GUI-2 through GUI-5 establish one bounded Apple-Silicon internal developer path from native selection and real OCCT analysis through revision-bound review, complete inputs, cancellation/recovery, and deterministic USD 702 trace; Ubuntu and Windows pass equivalent configured native-host smokes, and Ubuntu also passes an unsigned extracted Debian package with verified native runtime, real retained-session STEP analysis, bounded launch, and exact-payload picker/analysis interaction on one virtual configuration. Durable repositories, signed/distributable three-OS packaging, a supported production importer, calibrated deployment, and a release package do not exist
- **Recently completed:** TASK-004 adds deterministic public `FIX-MESH-047/048`, isolating one reversed cube facet and two analytic tetrahedra sharing one non-manifold edge without self-intersection/coplanar ambiguity. Both remain unresolved-unit `NeedsReview` evidence and withhold volume/centroid under the existing STL-v2 topology/confidence policy; no parser/schema migration applies. Complete local validation passes; three-OS Rust CI run 32971021045 passes the preceding OPC checkpoint at exact commit `33ccc18`, while exact-commit hosted evidence for this topology slice is pending. The fixtures remain synthetic parser evidence—not representative exporter, worker/desktop, supported-importer, or release evidence.
...
- **Geometry:** Intake/units/hash/healing/validity/basic measurements/stock/feature/access/setup stages are specified. Fourteen STL, thirty-four 3MF, one OCCT-generated analytic STEP, one manually authored analytic STEP, and one invalid-entity STEP fixture exist. The optional local worker produces provisional exact-B-rep measurements through OCCT; bounded ASCII/binary STL plus 3MF comparison analyzers reproduce nineteen successful mesh fixtures and exact rejections for twenty-nine adversarial mesh inputs. Success evidence covers all six Core units/default, direct through two-link linear component transforms, bounded model-metadata presence, three isolated legal OPC variations converted to canonical millimetres, and separate STL open-boundary, inconsistent-winding, non-manifold-edge, and self-intersection states with categorical confidence and conditional measurement withholding. Failure evidence covers selected graph/reference/union/metadata/material/extension/archive/XML/resource threats, binary framing/attribute/numeric/quota failures, and ASCII invalid-text/grammar/empty/degenerate failures without geometry authority. Most format fixtures remain derived from the same analytic cube or project-authored tetrahedra rather than independent CAD-tool or representative-shop evidence. Developer runtimes can be assembled and content-verified independently of Cargo output on evidenced Apple-Silicon, Ubuntu-x86_64, and Windows-x64 configurations. Supported general DAG/material semantics, broader exporter/legal-package coverage, production mesh tolerance, reviewed corpus accuracy, sandboxing, signed three-OS packaging, and product support remain absent.
...
| STL/3MF mesh import/validation | In Progress: bounded ASCII/binary STL plus thirteen successful direct/bounded-linear-component-chain 3MF package/unit/transform/model-metadata/OPC-layout fixtures, governed open/reversed/non-manifold/intersecting STL evidence, and twenty-nine typed adversarial rejection fixtures pass; categorical confidence and exact-predicate detected/indeterminate intersection states are explicit, while supported general DAG/material semantics, broader exporter/package and representative malformed entity/geometry cases, production welding/tolerance policy, worker/desktop integration, and representative corpus remain open | ADR-0004 and TASK-004 |
