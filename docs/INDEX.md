...
- **Next actions:** Continue TASK-004 with broader component-graph, object/item/vendor-metadata, and adversarial cases, additional binary/malformed cases, self-intersection/confidence policy, and worker/desktop integration without promoting the synthetic slices to support. Obtain GUI-1 fixture review; add interactive Windows evidence, Windows/macOS package integration, macOS/Windows parser egress denial, general filesystem containment, target dynamic-link/license/SBOM review, and signed application integration. Full TASK-005 durable desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
...
- **Geometry:** Intake/units/hash/healing/validity/basic measurements/stock/feature/access/setup stages are specified. Three STL, nine 3MF, one OCCT-generated analytic STEP, one manually authored analytic STEP, and one invalid-entity STEP fixture exist. The optional local worker produces provisional exact-B-rep measurements through OCCT; bounded ASCII/binary STL plus 3MF comparison analyzers reproduce twelve mesh fixtures, with STL units unresolved and persisted 3MF evidence for all six Core units, the normative default, direct/single-component transforms, and bounded model-metadata presence converted to canonical millimetres. The 3MF corpus is multiple deterministic encodings of the same analytic cube rather than independent representative geometry. Developer runtimes can be assembled and content-verified independently of Cargo output on evidenced Apple-Silicon, Ubuntu-x86_64, and Windows-x64 configurations. Broad component/object/item/vendor-metadata coverage, reviewed corpus accuracy, sandboxing, signed three-OS packaging, and product support remain absent.
...
| STL/3MF mesh import/validation | In Progress: bounded ASCII/binary STL plus nine persisted direct and single-reference-component 3MF package/unit/transform/model-metadata fixtures pass, including every Core unit declaration and the normative default; broader component graphs/unions, object/item/vendor metadata, adversarial layouts, binary cases, confidence/self-intersection policy, worker/desktop integration, and representative corpus remain open | ADR-0004 and TASK-004 |
