...
- **Overall status:** Pre-alpha engineering foundation. TASK-001 and TASK-002 calculation/rate mechanics pass local and three-OS CI; TASK-003 has cross-platform worker contracts, partial OS containment including poll-bounded aggregate private-workspace output monitoring and Linux `x86_64`/`aarch64` parser-phase socket/process syscall denial, provisional OCCT byte-stream measurements, one manually authored analytic STEP fixture, and host-verified developer-runtime evidence on Apple Silicon, Ubuntu x86_64, and Windows x64. TASK-004 now has bounded ASCII/binary STL and direct/linear-component-chain 3MF comparison slices for seventeen successful governed synthetic mesh fixtures plus twenty-nine persisted rejected mesh inputs. Versioned exact-predicate intersection evidence and categorical reason-coded confidence cap validated mesh at `Low`; unresolved units, topology defects, detected intersection, or indeterminate coplanar overlap require `NeedsReview`, and intersection uncertainty withholds enclosed volume/centroid. The thirty-four 3MF packages cover every Core length-unit declaration plus the normative millimetre default, direct through two-link linear transform/provenance cases, bounded well-known model metadata presence without retained content, three isolated legal OPC variations, and exact fail-closed graph/structural/metadata/extension/archive/XML/resource diagnostics; four rejected binary STL derivatives add exact framing/attribute/numeric/quota evidence, and four rejected ASCII STL streams add exact invalid-text/grammar/empty/degenerate evidence. Supported general DAG/material semantics, broader legal OPC/exporter coverage, production mesh tolerance, worker/desktop integration, and support remain open. GUI-2 through GUI-5 establish one bounded Apple-Silicon internal developer path from native selection and real OCCT analysis through revision-bound review, complete inputs, cancellation/recovery, and deterministic USD 702 trace; Ubuntu and Windows pass equivalent configured native-host smokes, and Ubuntu also passes an unsigned extracted Debian package with verified native runtime, real retained-session STEP analysis, bounded launch, and exact-payload picker/analysis interaction on one virtual configuration. Durable repositories, signed/distributable three-OS packaging, a supported production importer, calibrated deployment, and a release package do not exist
- **Recently completed:** TASK-004 adds deterministic public `FIX-MESH-044`–`046`, three isolated successful 3MF/OPC variants covering a default `.model` content-type mapping, an alternate explicit internal StartPart/model-part path, and Store compression. Each preserves the governed translated 10 mm cube evidence and categorical `Low` confidence. This persists existing parser-v6 behavior without changing the accepted boundary or schema, so no parser/schema migration applies. Three-OS Rust CI run 32751472560 passes the preceding malformed-ASCII checkpoint at exact commit `36b1b58`; the current OPC slice passes complete local closeout and awaits exact-commit hosted evidence. It is not general exporter, worker/desktop, representative-corpus, supported-importer, signing, legal, or distribution evidence.
...
- **Next actions:** Continue TASK-004 with broader representative malformed geometry, broader exporter/legal-package evidence, and production-tolerance research, then worker/desktop integration without promoting the synthetic slices to support. Obtain GUI-1 fixture review; add interactive Windows evidence, Windows/macOS package integration, macOS/Windows parser egress denial, general filesystem containment, target dynamic-link/license/SBOM review, and signed application integration. Full TASK-005 durable desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
...
- **Geometry:** Intake/units/hash/healing/validity/basic measurements/stock/feature/access/setup stages are specified. Twelve STL, thirty-four 3MF, one OCCT-generated analytic STEP, one manually authored analytic STEP, and one invalid-entity STEP fixture exist. The optional local worker produces provisional exact-B-rep measurements through OCCT; bounded ASCII/binary STL plus 3MF comparison analyzers reproduce seventeen successful mesh fixtures and exact rejections for twenty-nine adversarial mesh inputs. Success evidence covers all six Core units/default, direct through two-link linear component transforms, bounded model-metadata presence, three isolated legal OPC variations converted to canonical millimetres, and one self-intersecting analytic two-tetrahedron mesh with categorical confidence and withheld closed measurements. Failure evidence covers selected graph/reference/union/metadata/material/extension/archive/XML/resource threats, binary framing/attribute/numeric/quota failures, and ASCII invalid-text/grammar/empty/degenerate failures without geometry authority. Most format fixtures remain derived from the same analytic cube; the tetrahedra are project-authored rather than independent CAD-tool or representative-shop evidence. Developer runtimes can be assembled and content-verified independently of Cargo output on evidenced Apple-Silicon, Ubuntu-x86_64, and Windows-x64 configurations. Supported general DAG/material semantics, broader exporter/legal-package coverage, production mesh tolerance, reviewed corpus accuracy, sandboxing, signed three-OS packaging, and product support remain absent.
...
| STL/3MF mesh import/validation | In Progress: bounded ASCII/binary STL plus thirteen successful direct/bounded-linear-component-chain 3MF package/unit/transform/model-metadata/OPC-layout fixtures, one governed self-intersecting mesh, and twenty-nine typed adversarial rejection fixtures pass; categorical confidence and exact-predicate detected/indeterminate intersection states are explicit, while supported general DAG/material semantics, broader exporter/package and representative malformed entity/geometry cases, production welding/tolerance policy, worker/desktop integration, and representative corpus remain open | ADR-0004 and TASK-004 |
