...
- **Next actions:** Obtain GUI-1 fixture review; let corrected Windows run 31613259785 reach the separated header/dependency inspection and accept Windows evidence only if x64 PE, complete OCCT import closure, and configured host launch all pass. Linux Debian-contained shell evidence now passes run 31612819644; next package work is verified-native-runtime integration plus interactive dialog/portal/accessibility evidence, not another shell-only build. Add macOS/Windows parser egress denial, general filesystem containment, target dynamic-link/license/SBOM review, and signed application integration while closing remaining TASK-003 containment. TASK-004 mesh import, full TASK-005 durable desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
...
- **Geometry:** Intake/units/hash/healing/validity/basic measurements/stock/feature/access/setup stages are specified. Two STL, one OCCT-generated analytic STEP, one manually authored analytic STEP, and one invalid-entity STEP fixture exist. The optional local worker produces provisional exact-B-rep measurements through OCCT; developer runtimes can be assembled and content-verified independently of Cargo output on the evidenced Apple-Silicon and Ubuntu-x86_64 configurations. Reviewed corpus accuracy, sandboxing, signed three-OS packaging, and product support remain absent.
...
| OCCT STEP import/measurement/healing/package corpus | Exact 8.0.0 Apple-Silicon and Ubuntu-x86_64 construction plus supervised generated-cube and manually authored-prism measurements pass; fixture review, broader independent corpus, Windows construction, three-OS artifacts, legal/package evidence pending | ADR-0002 and TASK-003 |
| Isolated geometry-worker IPC/crash/resource boundary | Bounded subprocess, consumed already-open grant, identity-bound manifest v2, exact resource allowlisting, worker byte verification, controlled workspaces/output, cooperative/forced cancellation, poll-bounded aggregate workspace-output monitoring, Linux parser socket/process syscall denial, and partial Unix/Linux/Windows resource controls pass. The validated provisional snapshot binds claimed bytes to the source. Local Apple-Silicon and hosted Ubuntu-x86_64 tooling constructs and fingerprints the pinned OCCT root before strict native checks; Ubuntu additionally passes runtime/link verification and the configured host smoke under seccomp. A separate Ubuntu 22.04 unsigned Debian payload survives the bounded shell smoke without native analysis. Fixture review, representative mid-transfer proof, internally uninterruptible phase bounds, macOS/Windows parser egress denial, general filesystem sandboxing and hard filesystem quotas, hard macOS memory, hostile macOS descendants, signed/native-runtime-integrated packaging, interactive Linux acceptance, and completed Windows native evidence remain pending | ADR-0005 and TASK-003 |
