...
- **Overall status:** Pre-alpha engineering foundation. TASK-001 and TASK-002 calculation/rate mechanics pass local and three-OS CI; TASK-003 has cross-platform worker contracts, partial OS containment including poll-bounded aggregate private-workspace output monitoring and Linux `x86_64`/`aarch64` parser-phase socket/process syscall denial, provisional OCCT byte-stream measurements, one manually authored analytic STEP fixture, and host-verified developer-runtime evidence on Apple Silicon, Ubuntu x86_64, and Windows x64. TASK-004 now has bounded ASCII/binary STL and direct/linear-component-chain 3MF comparison slices for fourteen successful governed synthetic mesh fixtures plus twenty-five persisted rejected mesh inputs. Versioned exact-predicate intersection evidence and categorical reason-coded confidence cap validated mesh at `Low`; unresolved units, topology defects, detected intersection, or indeterminate coplanar overlap require `NeedsReview`, and intersection uncertainty withholds enclosed volume/centroid. The thirty-one 3MF packages cover every Core length-unit declaration plus the normative millimetre default, direct through two-link linear transform/provenance cases, bounded well-known model metadata presence without retained content, and exact fail-closed graph/structural/metadata/extension/archive/XML/resource diagnostics; four rejected binary STL derivatives add exact framing/attribute/numeric/quota evidence. Supported general DAG/material semantics, production mesh tolerance, worker/desktop integration, and support remain open. GUI-2 through GUI-5 establish one bounded Apple-Silicon internal developer path from native selection and real OCCT analysis through revision-bound review, complete inputs, cancellation/recovery, and deterministic USD 702 trace; Ubuntu and Windows pass equivalent configured native-host smokes, and Ubuntu also passes an unsigned extracted Debian package with verified native runtime, real retained-session STEP analysis, bounded launch, and exact-payload picker/analysis interaction on one virtual configuration. Durable repositories, signed/distributable three-OS packaging, a supported production importer, calibrated deployment, and a release package do not exist
- **Recently completed:** TASK-004 adds project-authored public `FIX-MESH-039`, two individually closed/wound tetrahedral shells with intersecting surfaces. STL v2 and 3MF v6 retain separately versioned exact-predicate self-intersection and categorical confidence policies. The fixture remains manifold/watertight/consistently wound but reports `detected`, `NeedsReview`, and unavailable volume/centroid; an inline coplanar case reports `indeterminate`. Successful fixture schema v3 now requires self-intersection plus confidence/reasons, rejects mesh confidence above `Low`, and forbids closed measurements for a self-intersecting mesh. This is a preproduction contract migration with no customer/persisted results; old success schemas and parser outputs are not silently upgraded. Replacement Rust CI run 32745701968 passes Windows, macOS, and Ubuntu at exact correction commit `3cfe34d`, closing the ZIP-name portability regression. The current intersection slice remains local pending its exact-commit run and is not installation, worker/desktop integration, a production tolerance policy, supported importer, signing, legal approval, or distribution approval.
...
- **Next actions:** Continue TASK-004 with broader malformed entity/geometry evidence and production-tolerance research, then worker/desktop integration without promoting the synthetic slices to support. Obtain GUI-1 fixture review; add interactive Windows evidence, Windows/macOS package integration, macOS/Windows parser egress denial, general filesystem containment, target dynamic-link/license/SBOM review, and signed application integration. Full TASK-005 durable desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
...
- **Geometry:** Intake/units/hash/healing/validity/basic measurements/stock/feature/access/setup stages are specified. Eight STL, thirty-one 3MF, one OCCT-generated analytic STEP, one manually authored analytic STEP, and one invalid-entity STEP fixture exist. The optional local worker produces provisional exact-B-rep measurements through OCCT; bounded ASCII/binary STL plus 3MF comparison analyzers reproduce fourteen successful mesh fixtures and exact rejections for twenty-five adversarial mesh inputs. Success evidence covers all six Core units/default, direct through two-link linear component transforms, bounded model-metadata presence converted to canonical millimetres, and one self-intersecting analytic two-tetrahedron mesh with categorical confidence and withheld closed measurements. Failure evidence covers selected graph/reference/union/metadata/material/extension/archive/XML/resource threats plus binary framing/attribute/numeric/quota failures without geometry authority. Most format fixtures remain derived from the same analytic cube; the tetrahedra are project-authored rather than independent CAD-tool or representative-shop evidence. Developer runtimes can be assembled and content-verified independently of Cargo output on evidenced Apple-Silicon, Ubuntu-x86_64, and Windows-x64 configurations. Supported general DAG/material semantics, production mesh tolerance, reviewed corpus accuracy, sandboxing, signed three-OS packaging, and product support remain absent.
...
| STL/3MF mesh import/validation | In Progress: bounded ASCII/binary STL plus ten successful direct/bounded-linear-component-chain 3MF package/unit/transform/model-metadata fixtures, one governed self-intersecting mesh, and twenty-five typed adversarial rejection fixtures pass; categorical confidence and exact-predicate detected/indeterminate intersection states are explicit, while supported general DAG/material semantics, broader malformed entity/geometry cases, production welding/tolerance policy, worker/desktop integration, and representative corpus remain open | ADR-0004 and TASK-004 |
