...
- **Overall status:** Pre-alpha engineering foundation. TASK-001 and TASK-002 calculation/rate mechanics pass local and three-OS CI; TASK-003 has cross-platform worker contracts, partial OS containment, Apple Silicon provisional OCCT byte-stream measurements, reproducible pinned local construction, and one manually authored analytic STEP fixture. GUI-4 can now connect a configured developer worker to the native application service and display path-free provisional geometry facts while retaining an ephemeral draft session, but no GUI estimate, durable repository, supported production importer, calibrated deployment, or release package exists
- **Recently completed:** GUI-4's second slice adds desktop contract v2, an asynchronous token-only analysis command, explicit developer-worker/workspace/OCCT configuration, session-scoped authorization/audit, native `DraftEstimateSession` retention, path-free provisional geometry DTOs, and deliberate unavailable/failure rendering. The positive adapter path is tested without CAD authority in the UI; an actual local OCCT runtime was unavailable for this session. GUI-1 implementation remains ready for formal fixture review.
...
- **Next actions:** Complete GUI-4's explicit unit/warning review, manual estimate inputs, pinned shop-owned rates/pricing policy, deterministic result trace, and cancellation over the connected native session, then GUI-5 end-to-end/accessibility evidence, while continuing GUI-1 fixture review and TASK-003 containment/native construction. TASK-004 mesh import, full TASK-005 desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
...
|---|---|---|
| Tauri 2 + Leptos desktop shell, dense table, accessibility, wgpu/PDF integration | GUI-4 contract/host/UI analysis adapter and responsive provisional-facts workspace pass locally; configured native runtime smoke, estimate forms, cancellation, dense table, full accessibility matrix, wgpu/PDF integration, three-OS package evidence, signing, and production adoption remain pending | ADR-0001/0003 and TASK-005 |
