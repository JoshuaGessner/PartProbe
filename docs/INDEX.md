...
- **Overall status:** Pre-alpha engineering foundation. TASK-001 and TASK-002 calculation/rate mechanics pass local and three-OS CI; TASK-003 has cross-platform worker contracts, partial OS containment including poll-bounded aggregate private-workspace output monitoring and Linux `x86_64`/`aarch64` parser-phase socket/process syscall denial, provisional OCCT byte-stream measurements, one manually authored analytic STEP fixture, and host-verified developer-runtime evidence on Apple Silicon, Ubuntu x86_64, and Windows x64. TASK-004 now has bounded ASCII/binary STL and direct/linear-component-chain 3MF comparison slices for thirteen governed synthetic mesh fixtures. Ten persisted 3MF packages cover every Core length-unit declaration plus the normative millimetre default, direct through two-link linear transform/provenance cases, bounded well-known model metadata presence without retained content, ZIP/OPC validation, shared conditional closed-volume evidence, and fail-closed archive/XML/entity/object/component/metadata cases; branching/unions, object/item/vendor metadata, worker/desktop integration, and support remain open. GUI-2 through GUI-5 establish one bounded Apple-Silicon internal developer path from native selection and real OCCT analysis through revision-bound review, complete inputs, cancellation/recovery, and deterministic USD 702 trace; Ubuntu and Windows pass equivalent configured native-host smokes, and Ubuntu also passes an unsigned extracted Debian package with verified native runtime, real retained-session STEP analysis, bounded launch, and exact-payload picker/analysis interaction on one virtual configuration. Durable repositories, signed/distributable three-OS packaging, a supported production importer, calibrated deployment, and a release package do not exist
- **Recently completed:** TASK-004 `partprobe-3mf-spike-v4` adds deterministic `FIX-MESH-013` with a two-link bounded linear component chain. The parser retains every link's object/reference IDs and exact transform, applies them leaf-to-build before the build transform, and rejects branching, non-immediate references, unions, and depth/count overrun. Three-OS Rust CI run 32613812143 passes the preceding metadata checkpoint at exact commit `969def9`; run 32613154817 passes the persisted Core-unit corpus at `3f6e6bf`, while full native Linux package run 32610311910 passes the corrected exact application-owned geometry-label gate at `1177e10`. This remains internal developer evidence, not installation, full accessibility/usability, supported importer, signing, legal approval, or distribution approval.
...
- **Next actions:** Continue TASK-004 with adversarial branching/union/component-reference, object/item/vendor-metadata, and archive-layout cases, additional binary/malformed cases, self-intersection/confidence policy, and worker/desktop integration without promoting the synthetic slices to support. Obtain GUI-1 fixture review; add interactive Windows evidence, Windows/macOS package integration, macOS/Windows parser egress denial, general filesystem containment, target dynamic-link/license/SBOM review, and signed application integration. Full TASK-005 durable desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
...
- **Geometry:** Intake/units/hash/healing/validity/basic measurements/stock/feature/access/setup stages are specified. Three STL, ten 3MF, one OCCT-generated analytic STEP, one manually authored analytic STEP, and one invalid-entity STEP fixture exist. The optional local worker produces provisional exact-B-rep measurements through OCCT; bounded ASCII/binary STL plus 3MF comparison analyzers reproduce thirteen mesh fixtures, with STL units unresolved and persisted 3MF evidence for all six Core units, the normative default, direct through two-link linear component transforms, and bounded model-metadata presence converted to canonical millimetres. The 3MF corpus is multiple deterministic encodings of the same analytic cube rather than independent representative geometry. Developer runtimes can be assembled and content-verified independently of Cargo output on evidenced Apple-Silicon, Ubuntu-x86_64, and Windows-x64 configurations. Branching/union/object/item/vendor-metadata coverage, reviewed corpus accuracy, sandboxing, signed three-OS packaging, and product support remain absent.
...
| STL/3MF mesh import/validation | In Progress: bounded ASCII/binary STL plus ten persisted direct and bounded-linear-component-chain 3MF package/unit/transform/model-metadata fixtures pass, including every Core unit declaration and the normative default; branching/unions, object/item/vendor metadata, adversarial layouts, binary cases, confidence/self-intersection policy, worker/desktop integration, and representative corpus remain open | ADR-0004 and TASK-004 |
