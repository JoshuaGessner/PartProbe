...
- **Overall status:** Pre-alpha engineering foundation. TASK-001 and TASK-002 calculation/rate mechanics pass local and three-OS CI; TASK-003 has cross-platform worker contracts, partial OS containment including poll-bounded aggregate private-workspace output monitoring and Linux `x86_64`/`aarch64` parser-phase socket/process syscall denial, provisional OCCT byte-stream measurements, one manually authored analytic STEP fixture, and host-verified developer-runtime evidence on Apple Silicon, Ubuntu x86_64, and Windows x64. TASK-004 now has bounded ASCII/binary STL and direct/linear-component-chain 3MF comparison slices for thirteen successful governed synthetic mesh fixtures plus twenty-five persisted rejected mesh inputs. The thirty-one 3MF packages cover every Core length-unit declaration plus the normative millimetre default, direct through two-link linear transform/provenance cases, bounded well-known model metadata presence without retained content, and exact fail-closed graph/structural/metadata/extension/archive/XML/resource diagnostics; four rejected binary STL derivatives add exact framing/attribute/numeric/quota evidence. Supported general DAG/material semantics, worker/desktop integration, and support remain open. GUI-2 through GUI-5 establish one bounded Apple-Silicon internal developer path from native selection and real OCCT analysis through revision-bound review, complete inputs, cancellation/recovery, and deterministic USD 702 trace; Ubuntu and Windows pass equivalent configured native-host smokes, and Ubuntu also passes an unsigned extracted Debian package with verified native runtime, real retained-session STEP analysis, bounded launch, and exact-payload picker/analysis interaction on one virtual configuration. Durable repositories, signed/distributable three-OS packaging, a supported production importer, calibrated deployment, and a release package do not exist
- **Recently completed:** TASK-004 adds deterministic public `FIX-MESH-034`–`038`: malformed-model-XML, forbidden-document-type, archive-entry-count, non-finite-binary-normal, and binary-triangle-quota rejection inputs. Typed failure expectations pin each source hash and exact sanitized diagnostic while forbidding geometry evidence. The accepted boundaries and diagnostics are unchanged, so 3MF parser v5 and binary STL parser v1 remain pinned and no migration applies. Rust CI run 32740425239 at exact implementation commit `8eb2dff` passes macOS and Ubuntu but stops in Windows Python tooling because `ZipFile.namelist()` normalizes a governed backslash filename on that host even though deterministic reproduction succeeds. The corrected test inspects the two raw local/central ZIP filename fields used by archive policy instead of the host-normalized reader projection; the governed package and hash remain unchanged. Three-OS run 32734198822 remains the last fully passing default matrix pending replacement CI. This is parser-level rejection evidence, not installation, worker/desktop integration, full accessibility/usability, supported importer, signing, legal approval, or distribution approval.
...
- **Next actions:** Continue TASK-004 with self-intersection/confidence policy and broader malformed entity/geometry evidence, then worker/desktop integration without promoting the synthetic slices to support. Obtain GUI-1 fixture review; add interactive Windows evidence, Windows/macOS package integration, macOS/Windows parser egress denial, general filesystem containment, target dynamic-link/license/SBOM review, and signed application integration. Full TASK-005 durable desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
...
- **Geometry:** Intake/units/hash/healing/validity/basic measurements/stock/feature/access/setup stages are specified. Seven STL, thirty-one 3MF, one OCCT-generated analytic STEP, one manually authored analytic STEP, and one invalid-entity STEP fixture exist. The optional local worker produces provisional exact-B-rep measurements through OCCT; bounded ASCII/binary STL plus 3MF comparison analyzers reproduce thirteen successful mesh fixtures and exact rejections for twenty-five adversarial mesh inputs. Success evidence covers all six Core units/default, direct through two-link linear component transforms, and bounded model-metadata presence converted to canonical millimetres; failure evidence covers selected graph/reference/union/metadata/material/extension/archive/XML/resource threats plus binary framing/attribute/numeric/quota failures without geometry authority. The corpus remains derived from the same analytic cube rather than independent representative geometry. Developer runtimes can be assembled and content-verified independently of Cargo output on evidenced Apple-Silicon, Ubuntu-x86_64, and Windows-x64 configurations. Supported general DAG/material semantics, reviewed corpus accuracy, sandboxing, signed three-OS packaging, and product support remain absent.
...
| STL/3MF mesh import/validation | In Progress: bounded ASCII/binary STL plus ten successful direct/bounded-linear-component-chain 3MF package/unit/transform/model-metadata fixtures and twenty-five typed adversarial mesh rejection fixtures pass; every Core unit/default plus selected graph/reference/union/metadata/material/extension/archive/XML/resource and binary framing/attribute/numeric/quota failures are persisted, while supported general DAG/material semantics, broader malformed entity/geometry cases, confidence/self-intersection policy, worker/desktop integration, and representative corpus remain open | ADR-0004 and TASK-004 |
