...
- **Overall status:** Pre-alpha engineering foundation. TASK-001 and TASK-002 calculation/rate mechanics pass local and three-OS CI; TASK-003 has cross-platform worker contracts, partial OS containment, Apple Silicon provisional OCCT byte-stream measurements, reproducible pinned local construction, and one manually authored analytic STEP fixture. GUI-4 contract v3 now provides the complete bounded session-only code path from configured analysis through explicit review/manual inputs/session rates/pricing to a deterministic path-free estimate trace, but configured native end-to-end evidence, durable repositories, a supported production importer, calibrated deployment, and a release package do not exist
- **Recently completed:** GUI-4's implementation slice adds token-bound cooperative cancellation and native `DraftEstimateSession` evaluation to the existing analysis adapter. The deliberate Leptos workflow requires canonical-unit/warning confirmation, complete manual inputs, five confirmed ephemeral developer rates, and a confirmed versioned pricing policy; missing or invalid values fail closed and the UI owns no formulas. A synthetic native adapter/session test reconciles to the expected USD 702 result. An actual local OCCT runtime was unavailable, so configured desktop-to-worker-to-estimate validation remains GUI-5 work. GUI-1 implementation remains ready for formal fixture review.
...
- **Next actions:** Complete GUI-5 configured-worker desktop smoke, cancellation/recovery, keyboard, and accessibility evidence over the implemented GUI-4 workflow, while continuing GUI-1 fixture review and TASK-003 containment/native construction. TASK-004 mesh import, full TASK-005 desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
...
- **Geometry:** Intake/units/hash/healing/validity/basic measurements/stock/feature/access/setup stages are specified. Two STL, one OCCT-generated analytic STEP, one manually authored analytic STEP, and one invalid-entity STEP fixture exist. The optional local worker produces provisional exact-B-rep measurements through OCCT; reviewed corpus accuracy, sandboxing, packaging, and product support remain absent.
...
|---|---|---|
| Tauri 2 + Leptos desktop shell, dense table, accessibility, wgpu/PDF integration | GUI-4 contract-v3 host/UI analysis, cancellation, explicit estimate-input, and deterministic-result workflow passes locally; configured native runtime smoke/recovery, dense table, full accessibility matrix, wgpu/PDF integration, three-OS package evidence, signing, and production adoption remain pending | ADR-0001/0003 and TASK-005 |
...
| Isolated geometry-worker IPC/crash/resource boundary | Bounded subprocess, consumed already-open grant, identity-bound manifest v2, exact resource allowlisting, worker byte verification, controlled workspaces/output, cooperative/forced cancellation, and partial Unix/Linux/Windows resource controls pass. The validated provisional snapshot binds claimed bytes to the source. Local tooling constructs and fingerprints the pinned Apple Silicon OCCT root before strict native checks, and generated plus manually authored analytic STEP evidence passes through adapter ABI v3 and the supervisor. Fixture review, representative mid-transfer proof, internally uninterruptible phase bounds, network/filesystem sandboxing, hard macOS memory, hostile Unix descendant, aggregate-output, packaging, and native Windows/Linux evidence remain pending | ADR-0005 and TASK-003 |
