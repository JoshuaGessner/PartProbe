...
- **Overall status:** Pre-alpha engineering foundation. TASK-001 and TASK-002 calculation/rate mechanics pass local and three-OS CI; TASK-003 has cross-platform worker contracts, partial OS containment, Apple Silicon provisional OCCT byte-stream measurements, reproducible pinned local construction, one manually authored analytic STEP fixture, and a separately verified developer runtime containing the worker plus complete observed OCCT closure. GUI-2 through GUI-5 establish one bounded Apple-Silicon internal developer path from native selection and real OCCT analysis through revision-bound review, complete inputs, cancellation/recovery, and deterministic USD 702 trace. Durable repositories, Windows/Linux native construction, signed/distributable packaging, a supported production importer, calibrated deployment, and a release package do not exist
- **Recently completed:** A fail-closed TASK-003 runtime assembler now pins construction provenance, copies and fingerprints the native worker plus all 23 Apple-Silicon OCCT library families, verifies relative paths/safe symlinks/no extra artifacts, and refuses overwrite. The opt-in real-host STEP-to-estimate smoke passes against this dedicated copy, so later feature-off Cargo builds can no longer replace the tested runtime. GUI-5 and GUI-4 remain complete only for their bounded internal checkpoints; GUI-1 implementation remains ready for formal fixture/security review.
