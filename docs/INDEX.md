...
- **Recently completed:** Three-OS Rust CI run 32610301376 passes the direct-3MF and exact-geometry-label checkpoint at `1177e10`, while short Linux package run 32607101403 passes the exact feature-off Debian payload and real XDG-portal picker. Locally, the next TASK-004 slice advances to `partprobe-3mf-spike-v2` with a reproducible component-transform 3MF cube, explicit object/component limits and Core reference checks, component-then-build order, optional provenance, and canonical millimetre evidence through shared mesh formulas. This remains internal developer evidence, not installation, full accessibility/usability, supported importer, signing, legal approval, or distribution approval. GUI-5 and GUI-4 remain complete only for their bounded internal checkpoints; GUI-1 implementation remains ready for formal fixture/security review.
