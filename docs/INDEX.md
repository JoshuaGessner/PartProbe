...
- **Next actions:** Run the new opt-in Linux native-package gate. The host now checks one fixed packaged resource child and reuses the verified-runtime boundary; the pending workflow must prove construction, package inclusion, extraction/re-verification, real STEP-to-USD-702 analysis, and exact package-executable survival together. Obtain GUI-1 fixture review, add interactive Linux/Windows dialog/accessibility evidence, macOS/Windows parser egress denial, general filesystem containment, target dynamic-link/license/SBOM review, and signed application integration while closing remaining TASK-003 containment. TASK-004 mesh import, full TASK-005 durable desktop/rate UX, TASK-006 persistence, and TASK-007 shop calibration remain required for the broader product slice.
