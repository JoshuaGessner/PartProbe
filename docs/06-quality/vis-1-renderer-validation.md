...
Complete VIS-1 by explicitly accepting the feature-gated Tauri child-WebView API in ADR-0003 or replacing it with a stable equivalent; add keyboard/view controls and a complete textual accessibility-equivalent inspector; implement device-loss recreation plus a visible non-GPU/limited fallback; and run broader scaling/resize plus Windows/Linux package evidence. Current timeout/occlusion/outdated handling is bounded, while a lost or validation-failed surface becomes visibly unavailable. VIS-2's [manifest, artifact decoder, protocol migration, supervisor claim, native STEP emission, and application-retention boundary](vis-2-display-scene-validation.md) are now validated on Apple Silicon. Desktop activation and renderer scene replacement are the critical path. Real stock cannot be shown until a versioned placement/orientation/per-side policy exists.
