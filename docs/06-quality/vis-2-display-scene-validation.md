...
> **Open questions:** Progressive levels, representative performance, and immutable editable stock candidates
...
- Desktop contract v12 preserves v11's optional path-free `scene_reference` behavior and adds one exact standard-view request plus the actual current-view result. No display geometry, path, CAD bytes, persistence field, or customer-data migration is introduced. Contract v11 remains the historical source-bound handoff checkpoint; v12 is an additive session wire migration only.
...
The desktop adapter requests schema v2 only when the deliberate `viewer-spike` internal-test build profile activates the viewer. It uses the fixed 0.1 mm/12 degree profile and the existing combined output ceiling; no process flag is required. After analysis, native session state returns a scene only for the exact retained selection and analysis identities. The adapter hands that validated value directly to `partprobe-model-viewer`; no raw-buffer reconstruction or WebView geometry DTO exists. A matching application proposal may subsequently add only its dimensions through the native `proposed-stock-display-placement-v1` path; display buffers remain absent from the contract.

The renderer concatenates the validated chunks into bounded indexed GPU buffers with checked offsets, frames the source from its actual finite decoded bounds, retains display normals for GPU lighting, and can add the matching proposal's exact rectangular dimensions under `proposed-stock-display-placement-v1`. That display-only policy source-axis aligns and centers the blank on renderer-owned source bounds, explicitly splitting total allowance equally between opposing faces while leaving standard size and availability unresolved. A new source clears the previous model and stock before analysis. Stale results cannot replace a newer selection. Scene-construction failure clears source buffers and reports display unavailable; fatal surface/device failure discards all GPU resources and leaves the same workspace in full-width text-only review. Both paths leave accepted analysis/estimate state unchanged.
...
Twelve ordinary renderer tests include analytic fully validated indexed source scenes, proving projected-extents fitting across all four views and multiple aspect ratios, index preservation, no stock before a proposal, exact centered/enclosing proposal stock, invalid-stock rejection, and the bounded surface/device fallback state machine. The `viewer-spike` desktop host passes 56 ordinary tests with five opt-in native/package smokes ignored. A fresh verified runtime explicitly passes the configured-worker desktop test: the governed prism travels through desktop schema-v2 request construction and retained native state as 24 vertices, 12 triangles, and 36 indices. Contract and host regressions preserve all twelve contract-v12 commands, map every standard-view value exactly, reject unconfigured camera changes, keep limited-mode text path-free and selection-bound, and prohibit geometry payloads, popup windows, or stale-selection acceptance.

Live Retina Apple-Silicon inspection exercises the current self-starting embedded-runtime developer `.app` through the real host-owned picker and native OCCT path. Before analysis, the selected 10 mm cube produces an empty native viewport with exact **Waiting for analysis** evidence, proving that a previous source scene is not reused. Native analysis returns the expected 10 × 10 × 10 mm bounds, 600 mm² surface area, 1,000 mm³ volume, and `(5, 5, 5)` mm centroid. The same-window Metal viewport then shows the complete source-bound cube with visible margins; automatic proposal preparation adds the exact 16.4 × 16.4 × 16.4 mm blank as translucent amber stock around shaded cyan source faces. Contract v12's Front button changes the native camera by mouse, Tab plus Return changes it to Top, and Isometric restores the source-bound view; the inspector exposes representation, units, dimensions, warnings, confidence, analysis reference, and proposal facts. The earlier inspection exposed and corrected longest-world-axis fitting, which clipped a rotated cube; the renderer now fits actual projected screen/depth extents, backed by the multi-view/aspect regression.

## Evidence not established

The native evidence is one fresh Apple-Silicon internal package, one coarse profile, two public synthetic STEP fixtures, one flattened root chunk, CPU-side renderer conversion, configured desktop retention, and one live source-bound Metal cube with standard-view/text-equivalent/proposed-stock evidence. Deterministic tests cover a preferred-adapter miss, device-loss observation, one bounded surface retry, and text-only failover, but no physical/driver loss has been forced. The evidence does not yet establish progressive loading, immutable editable stock candidates, standard stock/availability, representative-model performance, complete screen-reader/accessibility acceptance, in-session full-device recreation, successful fallback-adapter presentation on each target, Windows/Linux ABI-v1 display behavior, signed distribution, or supported importer status.

## Next gate

Force the implemented loss/no-adapter paths on declared platforms, decide whether full in-session device recreation is required, and complete screen-reader/focus evidence required by VIS-1. VIS-4 must introduce immutable stock-candidate revisions before editing or adopting dimensions that differ from the exact centered proposal. Add representative-model performance plus Linux/Windows/package evidence before promotion.
