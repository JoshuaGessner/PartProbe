...
Current TASK-004 evidence adds path-free, quota-bounded ASCII/binary STL and ZIP/OPC-bounded 3MF comparison analyzers over twelve public synthetic mesh fixtures. All formats share deterministic triangle bounds/area/manifold/closure/winding/conditional-volume measurement. STL units remain explicitly unresolved. Nine 3MF fixtures persist all six Core declarations plus the normative default, direct and single-reference component object provenance, component-then-build transform order, bounded well-known model-level metadata presence without retaining content, canonical-millimetre measurements, package relationship/content-type validation, and minimized archive/XML/entity/object/component/metadata/extension failure cases. The corpus is multiple encodings of one analytic cube, not nine independent geometry cases. Broader binary/malformed cases, component graphs/unions, object/item/vendor metadata, materials, self-intersection policy, approximate-confidence scoring, worker/desktop integration, representative geometry, and supported-importer acceptance remain open; see `task-004-validation.md`.
...
- 3MF with broader component graphs/transforms, object/item/vendor metadata and material layouts, archive traversal attempt, high compression ratio and unsupported required extension; all Core units/default and one well-known model-metadata layout now have persisted analytic fixtures.
