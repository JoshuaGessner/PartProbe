...
Current TASK-004 evidence adds path-free, quota-bounded ASCII/binary STL and ZIP/OPC-bounded 3MF comparison analyzers over five public synthetic mesh fixtures. All formats share deterministic triangle bounds/area/manifold/closure/winding/conditional-volume measurement. STL units remain explicitly unresolved. The two 3MF fixtures prove explicit-centimetre conversion, direct and single-reference component object provenance, component-then-build transform order, canonical-millimetre measurements, package relationship/content-type validation, and minimized archive/XML/entity/object/component/extension failure cases. Broader binary/malformed cases, persisted per-unit 3MF files, component graphs/unions, metadata/materials, self-intersection policy, approximate-confidence scoring, worker/desktop integration, representative geometry, and supported-importer acceptance remain open; see `task-004-validation.md`.
...
- 3MF with persisted fixtures for every supported unit, broader component graphs/transforms, archive traversal attempt, high compression ratio and unsupported required extension.
