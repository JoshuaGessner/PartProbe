...
Current TASK-004 evidence adds path-free, quota-bounded ASCII/binary STL and ZIP/OPC-bounded 3MF comparison analyzers over four public synthetic mesh fixtures. All formats share deterministic triangle bounds/area/manifold/closure/winding/conditional-volume measurement. STL units remain explicitly unresolved. The 3MF fixture proves explicit-centimetre conversion, one build translation, canonical-millimetre measurements, package relationship/content-type validation, and minimized archive/XML/entity/extension failure cases. Broader binary/malformed cases, every 3MF unit, components/metadata/materials, self-intersection policy, approximate-confidence scoring, worker/desktop integration, representative geometry, and supported-importer acceptance remain open; see `task-004-validation.md`.
...
- 3MF with every supported unit, components/transforms, archive traversal attempt, high compression ratio and unsupported required extension.
