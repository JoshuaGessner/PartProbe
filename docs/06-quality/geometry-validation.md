...
Current TASK-004 evidence adds path-free, quota-bounded ASCII/binary STL and ZIP/OPC-bounded 3MF comparison analyzers over thirteen public synthetic mesh fixtures. All formats share deterministic triangle bounds/area/manifold/closure/winding/conditional-volume measurement. STL units remain explicitly unresolved. Ten 3MF fixtures persist all six Core declarations plus the normative default, direct through two-link linear component-chain provenance, ordered leaf-to-build transform application, bounded well-known model-level metadata presence without retaining content, canonical-millimetre measurements, package relationship/content-type validation, and minimized archive/XML/entity/object/component/metadata/extension failure cases. The corpus is multiple encodings of one analytic cube, not ten independent geometry cases. Broader binary/malformed cases, branching component graphs/unions, object/item/vendor metadata, materials, self-intersection policy, approximate-confidence scoring, worker/desktop integration, representative geometry, and supported-importer acceptance remain open; see `task-004-validation.md`.
...
- 3MF with adversarial branching/union/non-immediate component references, object/item/vendor metadata and material layouts, archive traversal attempts, high compression ratio, and unsupported required extensions; all Core units/default, one two-link linear chain, and one well-known model-metadata layout now have persisted analytic fixtures.
