...
Current TASK-004 evidence adds path-free, quota-bounded ASCII and binary STL comparison analyzers over three public synthetic mesh fixtures. It reports triangle count, source-coordinate bounds/area, manifold/closure/winding state, and only reports enclosed volume/centroid for a closed consistently wound case. Binary selection requires exact record framing; unsupported attribute payloads and non-finite values fail closed. Units remain explicitly unresolved and diagnostics are sanitized. 3MF, broader binary/malformed cases, self-intersection policy, approximate-confidence scoring, desktop integration, representative geometry, and supported-importer acceptance remain open; see `task-004-validation.md`.
