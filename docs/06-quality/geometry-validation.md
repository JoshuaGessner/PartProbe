...
Current TASK-004 evidence adds path-free, quota-bounded ASCII/binary STL and ZIP/OPC-bounded 3MF comparison analyzers over seventeen successful public synthetic mesh fixtures plus twenty-nine persisted rejected mesh inputs. All accepted formats share deterministic triangle bounds/area/manifold/closure/winding, versioned exact-predicate self-intersection state, categorical reason-coded confidence, and conditional-volume measurement. STL units remain explicitly unresolved and therefore `NeedsReview`; validated unit-resolved 3MF mesh evidence reaches only `Low`. `FIX-MESH-039` proves that intersecting closed/wound shells remain `NeedsReview` and cannot expose volume/centroid, while an inline coplanar-overlap case remains explicitly `indeterminate`. Thirteen successful 3MF fixtures persist all six Core declarations/default, direct through two-link linear component-chain provenance, ordered transforms, bounded model metadata, three isolated legal OPC variations, and canonical measurements. Twenty-one adversarial 3MF packages pin exact no-evidence diagnostics for selected graph/reference/union/metadata/material/extension/archive/XML/resource threats; four binary STL inputs pin truncated framing, unsupported facet attributes, non-finite numbers, and triangle-quota rejection; and four ASCII STL inputs pin invalid-text, malformed-grammar, empty-mesh, and degenerate-geometry rejection. The main format corpus still derives from one analytic cube; the intersecting tetrahedra are a second project-authored analytic shape, not independent CAD-tool or representative machining evidence. Broader representative malformed entity/geometry cases, supported general DAG/material semantics, broader alternate legal package layouts, production welding/tolerance policy, worker/desktop integration, representative geometry, and supported-importer acceptance remain open; see `task-004-validation.md`.
