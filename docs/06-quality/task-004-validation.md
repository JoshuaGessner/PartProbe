...
Version `partprobe-3mf-spike-v3` validates an in-memory ZIP/OPC package before reading the selected model. It requires the exact OPC content-types and root-relationships parts, resolves one internal StartPart target, verifies the model content type, and never extracts a part to disk. It rejects encrypted, non-file, unsafe-name, duplicated/case-fold-ambiguous, externally targeted, unsupported-compression, malformed XML, document-type/general-reference, unsupported-required-extension, and unsupported-model-structure inputs. Limits cover original bytes, entry count, aggregate expanded bytes, per-XML bytes, compression ratio, vertices, triangles, objects, component references, and model-level metadata entries.

The bounded model contract accepts the 3MF Core namespace, every Core length unit plus its normative millimetre default, zero to the configured limit of unique well-known unprefixed model-level Core metadata entries, one leaf mesh object, optionally one later component object that references that mesh exactly once, and one build item. It enforces Core positive resource-ID bounds and prior-object component reference order. Vertices pass through the documented row-vector component transform, then the build transform, then canonical-millimetre conversion before entering the same format-neutral triangle measurement code as STL; this order follows the Consortium's [Core 1.4 model, metadata, matrix, component, and build rules](https://github.com/3MFConsortium/spec_core/blob/master/3MF%20Core%20Specification.md). Metadata text and names are never retained; evidence exposes only total and `preserve=true` counts and adds `THREE_MF_METADATA_NOT_INTERPRETED`. Evidence also distinguishes an explicit `unit` attribute from the Core default; retains leaf/component/build counts and package-local IDs; represents absent component evidence as `None`, not zero; and retains both exact 12-value source-unit transforms plus applied-state flags. Duplicate/unknown/vendor metadata, unsupported metadata attributes/types, object/item metadata groups, singular or reflected transforms, multiple component instances, nested component graphs, extra/unused objects, multiple build items, materials, and unknown structures fail visibly rather than being flattened, unioned, interpreted, or ignored.
...
The binary and 3MF fixtures are deterministically generated from `FIX-MESH-001`; their check modes prove all ten derived binary/package artifacts are reproducible before tests consume them, including decimal source coordinates for inch and foot. Focused validation passes seven STL tests, ten 3MF tests, four fixture-contract tests, and nine generator-tooling tests. Coverage includes the binary, direct, component, persisted-unit, and Core-metadata encodings; content-based STL selection; shared deterministic mesh measurements; all six Core units plus the normative default; explicit/default distinction; identity and component-then-build transforms with retained ID provenance; positive archive/XML/entity/object/component/metadata limits; Core resource-ID bounds; metadata content redaction, preserve counting, duplicate/name/boolean rejection; exact binary framing; external/traversal relationship rejection; unsupported required extensions/material attributes/units/structures; invalid component references; multiple component rejection; singular/reflected transform rejection; unsupported binary attributes; malformed/non-finite input; empty input; and degenerate/overflowing geometry. Strict geometry-import Clippy passes.

## Evidence boundary

This is an internal comparison spike, not a supported importer. Binary evidence covers one simple zero-attribute analytic fixture. 3MF evidence now persists all six Core unit declarations plus the normative default, one project-generated translated direct mesh, one single-reference component variant, and one standard model-level metadata layout. Every package is derived from the same analytic cube, not independent geometry or CAD-tool evidence. Component graphs/unions, multiple build items, object/item metadata groups, vendor names/types, materials, reflected transforms, alternate legal OPC layouts, and a representative malformed corpus remain absent. Metadata values are not interpreted as geometry, requirements, or estimating inputs. The spike does not recognize self-intersections, assign an authoritative mesh-confidence score, enter the geometry-worker/desktop flow, persist a model, or validate representative machining parts. Exact floating-point vertex identity is sufficient for these analytic fixtures but is not yet the reviewed production welding/tolerance policy. TASK-004 remains In Progress until the broader STL/3MF validity/failure matrix and acceptance evidence exist.
