...
The binary and 3MF fixtures are deterministically generated from `FIX-MESH-001`; their check modes prove the committed 684-byte, 1,072-byte, and 1,107-byte artifacts are reproducible before tests consume them. Focused validation passes seven STL tests, nine 3MF tests, four fixture-contract tests, and seven generator-tooling tests. Coverage includes all three golden encodings, content-based STL selection, shared deterministic mesh measurements, all six Core units plus the normative default, explicit/default distinction, component-then-build transform order and retained transform/ID provenance, positive archive/XML/entity/object/component limits, Core resource-ID bounds, exact binary framing, external/traversal relationship rejection, unsupported required extensions/material attributes/units/structures, invalid component references, multiple component rejection, singular/reflected transform rejection, unsupported binary attributes, malformed/non-finite input, empty input, and degenerate/overflowing geometry. Strict geometry-import Clippy passes.

## Evidence boundary

This is an internal comparison spike, not a supported importer. Binary evidence covers one simple zero-attribute analytic fixture. 3MF evidence covers one project-generated direct mesh/build item and one single-reference component variant, one explicit unit plus the normative default, and minimized in-memory mutations—not persisted unit-specific fixture files, component graphs/unions, multiple build items, metadata/materials, reflected transforms, alternate legal OPC layouts, or a representative malformed corpus. The spike does not recognize self-intersections, assign an authoritative mesh-confidence score, enter the geometry-worker/desktop flow, persist a model, or validate representative machining parts. Exact floating-point vertex identity is sufficient for these analytic fixtures but is not yet the reviewed production welding/tolerance policy. TASK-004 remains In Progress until the broader STL/3MF unit/validity/failure matrix and acceptance evidence exist.
