...
[USE-2 persistence validation](use-2-persistence-validation.md) adds partial TEST-007/014 evidence for empty schema-v1 bootstrap, schema-v1-to-v2 forward migration without defaults, checksummed migration failure, immutable rate/pricing/resource/settings versions, concurrent stale-writer rejection, save/reopen, backup/reopen, historical replay, and hash-tamper rejection. The bounded resource snapshot preserves one typed material/offer/stock/machine/runtime starter bundle and rejects a reused version with changed bytes. Crash/power-loss, a second supported prior migration, broader corruption, protected storage, remaining aggregates, activation/approval, and three-OS evidence remain pending.
