...
| `cargo test --workspace --all-targets --locked` | Pass: 66 runtime tests |
...
TASK-003 currently adds twenty-five default runtime tests: five geometry-core invariants, nine protocol/supervisor/output/containment cases, six subprocess-boundary cases, four fixture-contract cases, and one default-disabled adapter case. Twelve focused tests pass across the explicit local native adapter/worker commands.

## Cross-platform evidence

GitHub Actions run 30480866473 passes formatting, strict Clippy, all 65 pre-containment default runtime tests, and documentation tests on Windows, Linux, and macOS for commit `e80ccf6`. This proves the kernel-neutral contracts, bounded subprocess transport, one-use open source grants, capability/length/hash checks, deleted-path operation, fixed-name staging, fixture schemas, locked Rust dependencies, feature-off behavior, target-specific final-component link/reparse-point rejection, private job-directory creation/cleanup, and immutable output claim/hash/unlink behavior are cross-platform. The capability-root containment dependency and regression currently pass locally on macOS and still require a new three-OS CI run. Existing evidence does not prove application-service actor/project/classification authorization, direct worker handle transport, native OCCT linking, OS sandbox, durable controlled-store persistence, or packaging behavior; the focused native-feature evidence is Apple Silicon only.
...
- Implement the application/security-service decision point for actor, project membership, classification, record state, approved root, and allow/deny audit evidence; persist claimed output through the controlled derivative store with classification, authorization, retention, and audit evidence.
