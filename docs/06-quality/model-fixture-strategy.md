...
`FIX-MESH-004` is a deterministic 1,072-byte 3MF Core/OPC package derived from the same reviewed triangles. It stores the 10 mm cube as coordinates `0..1` with explicit centimetre units and applies one build-item translation of `(2, 3, 4)` source units. `partprobe-3mf-spike-v2` therefore must report canonical 10 mm extents, 600 mm² area, 1,000 mm³ volume, and translated centroid `(25, 35, 45)` mm while retaining mesh authority and the applied-transform evidence.

`FIX-MESH-005` is a deterministic 1,107-byte variant over the same leaf mesh. Object 2 references mesh object 1 exactly once with a positive-determinant X-scale of 2 and translation `(1, 2, 3)` source units; build item 2 then applies translation `(4, 5, 6)`. The governed component-then-build order yields canonical 20 × 10 × 10 mm extents, 1,000 mm² area, 2,000 mm³ volume, and centroid `(60, 75, 95)` mm. The parser must retain both object IDs and transforms, and must keep component absence distinct for `FIX-MESH-004`. The shared checked generator fixes ZIP entry order, timestamp, permissions, compression, relationship/content-type parts, and XML bytes for both packages. These fixtures do not establish persisted per-unit files, component graphs/unions, multiple build items, metadata/materials, representative parts, desktop/worker integration, or production-importer support.
