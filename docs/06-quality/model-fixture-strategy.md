...
`FIX-MESH-004` is a deterministic 1,072-byte 3MF Core/OPC package derived from the same reviewed triangles. It stores the 10 mm cube as coordinates `0..1` with explicit centimetre units and applies one build-item translation of `(2, 3, 4)` source units. `partprobe-3mf-spike-v3` therefore must report canonical 10 mm extents, 600 mm² area, 1,000 mm³ volume, and translated centroid `(25, 35, 45)` mm while retaining mesh authority and the applied-transform evidence.
