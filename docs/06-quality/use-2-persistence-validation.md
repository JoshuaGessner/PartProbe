...
Separate domain tests for the not-yet-persisted `shop-resource-catalog-v1` contract prove exact multi-entry round-trip, the single active-for-proposals ceiling, per-kind record bounds, duplicate identity/version rejection, reviewed-child requirements, and fail-closed selection/reference deserialization. They do not constitute persistence, authorization, desktop, proposal, or calculation evidence.
