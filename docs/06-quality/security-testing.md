...
GUI-4 intake evidence proves the application policy decision and append attempt occur before source resolution, fingerprinting, or analysis. An allowed source is hashed within its maximum-input bound from the same already-open grant later consumed by the worker, and the grant is rewound after hashing; a denied request against a nonexistent relative source returns the policy failure, appends one audit event, and never calls analysis. This avoids a desktop-host CAD read before authorization while preserving independent worker verification. It does not yet establish runtime desktop authorization policy, durable audit, worker configuration, or end-to-end UI analysis.
