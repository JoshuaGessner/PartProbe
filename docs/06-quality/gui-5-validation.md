...
On 2026-09-10, the configured-host STEP smoke passed in 4.10 seconds against a newly constructed and re-verified Apple-Silicon ABI-v4 runtime. That worker emits `geometry-step-analysis-v1`, retaining the unchanged exact snapshot used by the manual estimate path while adding source-bound 12 × 8 × 5 mm AABB evidence. The configured STL/3MF estimate-unavailable smoke also passed in 0.76 seconds. Worker SHA-256 is `4f7ab1803d15186917dce4a4eec0e8c7821fa8809bddc9c274f345faaa12d4c9`. Contract v10 subsequently exposes the three path-free bounds and adds the separate reviewed proposal/adoption path without altering the retained USD 702 manual fixture.

The final current-tree contract-v10 package is a 181 MiB unsigned arm64 `.app` with all 72 embedded ABI-v4 runtime artifacts reverified. Its package-contained worker passes both the retained manual STEP smoke and a second end-to-end proposal/adoption smoke over the real 12 × 8 × 5 mm fixture. The latter pins the settings/resource versions, derives a 15 × 11 × 7 mm blank, and retains explicit review and exclusions. GUI executable SHA-256 is `9a2e9b34551ef208460923aaa141962d4627a1c5f61f9be7c5c3dcd6cee80a6c`. The launch layout passes visual preflight. This supersedes the earlier local ABI-v3 package statement, but does not establish signed/notarized delivery, a human-driven populated-flow accessibility pass, or ABI-v4 Linux/Windows support.
