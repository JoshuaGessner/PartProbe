...
The additive developer candidate-session policy v1.0.0 retains at most 32 stock revisions and requires exact latest ID/revision/settings plus separately recorded review. It changes no calculation rule or input: tests prove candidate creation/review/successors preserve an already available deterministic baseline. The distinct candidate-adoption v1.0.0 rule now maps a precise recorded review plus explicit quantities/adopter evidence into sealed coarse inputs and evaluates them as a separate snapshot through the existing engine. Original inputs/results and all normal geometry/rate/pricing gates remain preserved. Its six zero-valued omissions require explicit recorded limitation acceptance; see [the adoption rule and migration decision](stock-selection-model.md#vis-4-developer-candidate-adoption-rule-v1). Production authorization and desktop activation remain future gates.
