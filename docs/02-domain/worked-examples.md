...
The application session-policy v1 test starts the prism's 20×12×8 mm candidate, records review on exact revision one, then creates an 18×12×8 mm successor. Revision one and its reviewer/time/reason remain retained; revision two has no inherited review. A stale revision-one edit or review fails without mutation. A separate executable test first obtains an available original-proposal quantity-three deterministic result using the TASK-002 synthetic rates, then proves candidate creation, review, and successor leave that exact baseline result unchanged. This is session sequencing/review evidence, not edited-stock estimate adoption or desktop UI acceptance.

The separate candidate-adoption v1.0.0 example reviews prism candidate revision one (20×12×8 mm) and explicitly adopts 2 delivered + 1 spare + 0 destructive items, with distinct adopter/time/reason. Purchased material becomes USD 0.132192 for make quantity three; cutting is 0.0015 hours per item, while setup/programming/load/quality remain 1/0.75/0.05/0.25 hours. Six named coarse exclusions remain visible. Ordinary unit/warning, applicable-rate, and pricing-policy gates still apply. Evaluation returns a separately pinned candidate result plus the unchanged original baseline state; superseding revision one prevents its adoption from generating a current result. Neither result is an approved quote or complete manufacturing-cost estimate.
