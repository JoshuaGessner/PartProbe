...
GUI-3 supplies a preliminary Apple-Silicon smoke. The rendered accessibility tree exposed the skip link, level-one/level-two headings, named application state, named native button, and source/estimate description lists; Tab produced visible focus, Return opened the picker, and Escape cancelled it. Reduced-motion and forced-color CSS fallbacks are present. The current Model & stock slice adds semantic keyboard view buttons, a path-free model/proposal text equivalent, an explicitly described control group, and a polite atomic visualization-status region. Estimate, Settings, and Model & stock now expose the active destination with `aria-current="page"`; after a successful user-initiated transition, one bounded animation-frame handoff focuses the new workspace's programmatically focusable heading, including the post-save return to Estimate. Failed or blocked native viewer transitions do not move focus. Each destination heading has a focus-visible treatment, and the existing skip link remains first in the document order. The refreshed unsigned macOS package confirms through the live platform accessibility bridge that pointer and keyboard transitions focus the exact **Shop rates and resources**, **Model & stock review**, and **Upload model** level-two headings. Keyboard activation visibly draws the heading focus ring, and the next Tab from the Estimate heading reaches **Choose model**. This is platform focus evidence, not a VoiceOver comprehension pass. If graphics startup or recovery fails, the same destination expands into text-only review, disables spatial camera actions, and preserves the analysis/proposal description lists instead of losing accepted work. The current Linux package follow-up requires the exact visible host-owned `Select model file` window, the XDG portal's unique exact showing `File Chooser Widget`, and the exact live `Files` table before navigation. It sends GTK's documented `/` location-popup binding from that owning file-list widget, requires the resulting unique focused location entry, enters the complete absolute synthetic STEP-fixture path in that native control, verifies the exact accessible text, and presses Return while the entry retains focus. Run 31971041371 proves the earlier flattened AT-SPI table-cell selection was not authoritative: GTK accepted its automatically selected first row. Run 31976213511 then proves complete-path submission selects the intended prism, closes the chooser, and exposes Analyze. Its final failure is a polling race between two accessibility traversals, not a wrong source. The corrected gate classifies exact, wrong, or ambiguous source state from one traversal snapshot, then requires picker closure and an enabled Analyze control. Contract v4 adds path-free mesh labels that state unknown units and withheld measurements; a live STL/3MF accessibility smoke remains pending. No path enters the WebView. No Orca/VoiceOver/Narrator comprehension session, automated contrast, zoom/scaling, high-DPI, complete estimate-form Linux/Windows workflow, or full acceptance pass has occurred, so TEST-012 and the non-negotiable examples remain open.
