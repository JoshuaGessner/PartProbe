...
GUI-3 supplies a preliminary Apple-Silicon smoke. The rendered accessibility tree exposed the skip link, level-one/level-two headings, named application state, named native button, and source/estimate description lists; Tab produced visible focus, Return opened the picker, and Escape cancelled it. Reduced-motion and forced-color CSS fallbacks are present. The Linux package follow-up uses AT-SPI to require the XDG portal's unique exact showing `File Chooser Widget`, focuses its exact Cancel control before keyboard location navigation, confirms the exact governed fixture selection, and applies a bounded native acceptance sequence: Return on the exact focused Select control, selected-file Return when the row is focusable, then the exact Select action. Each method stops only after chooser closure and PartProbe's `Model selected` transition. Run 31962878310 confirmed these portal-specific names after the backend correction but failed because the preceding harness still expected the old `Open File`/`Open` surface; hosted confirmation of the corrected semantics remains pending. No Orca/VoiceOver/Narrator session, automated contrast, zoom/scaling, high-DPI, complete estimate-form Linux/Windows workflow, or full acceptance pass has occurred, so TEST-012 and the non-negotiable examples remain open.
