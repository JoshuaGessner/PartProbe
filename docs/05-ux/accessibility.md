...
GUI-3 supplies a preliminary Apple-Silicon smoke. The rendered accessibility tree exposed the skip link, level-one/level-two headings, named application state, named native button, and source/estimate description lists; Tab produced visible focus, Return opened the picker, and Escape cancelled it. Reduced-motion and forced-color CSS fallbacks are present. The Linux package follow-up requires the exact visible host-owned `Select STEP model` window before keyboard input, then uses AT-SPI to require the XDG portal's unique exact showing `File Chooser Widget`, exact Cancel navigation anchor, exact governed fixture selection, and exact enabled Select control. Its bounded acceptance sequence is Return on Select, selected-file Return when the row is focusable, then the exact Select action; each method stops only after chooser closure and PartProbe's `Model selected` transition. Run 31964173142 proved AT-SPI child focus alone did not move X11 keyboard focus, so the current correction pins both layers. Hosted confirmation remains pending. No Orca/VoiceOver/Narrator session, automated contrast, zoom/scaling, high-DPI, complete estimate-form Linux/Windows workflow, or full acceptance pass has occurred, so TEST-012 and the non-negotiable examples remain open.
