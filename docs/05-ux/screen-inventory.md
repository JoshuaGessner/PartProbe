...
| Model and stock review | View model, proposed stock, integrity, and basic properties without giving the renderer calculation authority | 1 / VIS-1 phases A/B and VIS-2 source display implemented as developer preview; VIS-3–5 planned | Contract v12 exposes a clickable same-window destination, native viewport, return navigation, four semantic keyboard-operable standard views, and a path-free text equivalent for representation, units, dimensions, warnings, confidence, analysis reference, and available proposal facts only behind the exact developer feature/flag. The selected exact STEP's validated display derivative reaches the native renderer; geometry does not reach the WebView. Governed stock enclosure/placement and editing, machine-fit authority, exact candidate revisions, device-loss fallback, complete screen-reader evidence, and three-OS package acceptance remain |
