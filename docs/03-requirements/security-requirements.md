...
GUI-3 provides bounded configuration/test evidence for SEC-001 and SEC-002; GUI-4 established five analysis/estimate commands plus event listen/unlisten, contracts v5-v7 add exactly two Settings commands, contract v8 adds one separately permissioned catalog-selection activation request, and contract v9 adds one separately permissioned catalog-draft save request without broad frontend permissions. Contract v4 added tagged exact-B-rep/mesh response DTOs. No frontend network, shell, filesystem, dialog, opener, updater, upload, or remote-content permission is granted. Raw selected paths, worker authority, cancellation handle, complete controlled mesh provenance, the provisional analysis session, and the SQLite path remain native-owned; the WebView receives only path-free typed state. USE-2 catalog services preserve exact versions and fail closed. Schema v4 implements the activation audit port with content-minimized exact-version foreign keys, append-only triggers, normalized hash checks, correlation idempotency/collision handling, no-default migration, backup/reopen, and corruption rejection. The contract-v8 request has no actor/profile/time/correlation/path/storage authority; contract v9 has no selections/actor/profile/time/path/storage authority and bounds every record kind before typed mapping. The host derives trusted facts, ordinary startup has no actor, activation remains deny-all, and the WebView invokes neither catalog command. Successful decisions or saved drafts never grant estimate authority. The worker supervisor supplies portable poll-bounded aggregate job-output inspection, termination, and cleanup; Linux `x86_64`/`aarch64` adds parser syscall-denial evidence. These requirements remain incomplete because no authenticated production identity, role/member repository, shop-reviewed shipped catalog allow configuration, separately access-controlled or externally anchored authorization audit, or general security-event store exists; rate approvals and results remain developer-only, the manifest is unsigned and mutable, and macOS/Windows parser egress denial, general filesystem and hard-storage-quota tests, Linux aarch64 native behavior, navigation interception, signed packages, full diagnostics review, and three-OS window/package evidence remain pending.
