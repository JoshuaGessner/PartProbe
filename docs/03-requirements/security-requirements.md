...
GUI-3 provides bounded configuration/test evidence for SEC-001 and SEC-002; GUI-4 extends the exact application manifest/capability to three commands (`desktop_contract`, `select_model_source`, and token-only `analyze_model_source`) plus event listen/unlisten. No frontend network, shell, filesystem, dialog, opener, updater, upload, or remote-content permission is granted. Raw selected paths and the complete provisional analysis session remain in native process state; the webview receives only a leaf display name, opaque selection/session/analysis IDs, hashes, sanitized stage/warning codes, and provisional measurements. GUI-4 adds partial SEC-003/004/007 evidence: authorization and an append-only in-memory audit precede source resolution/fingerprinting, the same bounded open grant reaches the worker boundary, worker configuration is explicit and fail-closed, and missing configuration cannot fall back to ambient discovery or in-process parsing. These requirements remain incomplete because the policy and audit store are developer-session-only, runtime deployment policy, durable integrity-protected audit, configured desktop-to-native-worker smoke, OS-level egress tests, navigation interception, signed packages, full diagnostics review, and three-OS evidence are pending.
