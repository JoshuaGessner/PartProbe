...
GUI-3 provides bounded configuration/test evidence for SEC-001 and SEC-002; GUI-4 extends the exact application manifest/capability to five commands (`desktop_contract`, `select_model_source`, token-only `analyze_model_source`, token-bound `cancel_model_analysis`, and retained-session `evaluate_draft_estimate`) plus event listen/unlisten. Contract v4 retains that exact command/event boundary while adding tagged exact-B-rep/mesh response DTOs. No frontend network, shell, filesystem, dialog, opener, updater, upload, or remote-content permission is granted. Raw selected paths, worker authority, cancellation handle, complete controlled mesh provenance, and the provisional analysis session remain in native process state; the webview receives only a leaf display name, opaque selection/session/analysis IDs, hashes, sanitized warning/version/topology/confidence fields, optional decimal-text measurements, explicit estimate inputs, and path-free result traces. GUI-4 adds partial SEC-003/004/007 evidence: authorization and an append-only in-memory audit precede source resolution/fingerprinting, the same bounded open grant reaches the worker boundary, runtime/workspace configuration is explicit and fail-closed, cancellation is selection-scoped, selected-extension/content mismatches fail visibly, and estimate evaluation cannot bypass the typed application session or authorize mesh. The follow-on desktop checkpoint re-verifies the pinned runtime manifest, provenance, host, and full artifact closure at startup and accepts only manifest-derived worker/library paths. The worker supervisor additionally supplies portable, poll-bounded aggregate job-output inspection, termination, and owned-workspace cleanup evidence for SEC-004. Linux `x86_64`/`aarch64` workers add parser-phase seccomp evidence that socket and descendant creation are denied after authorized bytes are immutable; run 31555774851 adds configured Ubuntu x86_64 OCCT/internal-link compatibility under that boundary. Run 31613259785 adds configured Windows x64 runtime/PE-closure/host compatibility without a parser-egress boundary. GUI-5 exercised the configured exact boundary in the actual Apple-Silicon app, including path-redacted success, cancellation, bounded failure, and recovery. These requirements remain incomplete because policy, audit, rate approvals, and results are developer-session-only; the manifest is unsigned and mutable; configured real-worker desktop mesh evidence, runtime deployment policy, durable integrity-protected audit, macOS/Windows parser egress denial, general filesystem and hard-storage-quota tests, Linux aarch64 native behavior, navigation interception, signed packages, full diagnostics review, and three-OS window/package evidence are pending.
