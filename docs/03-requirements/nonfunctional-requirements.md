...
|---|---|---|---|
| REQ-NF-001 | Core workflows shall run on supported Windows, Linux, and macOS targets without platform-specific domain behavior. | TEST-008 CI/package matrix; GUI-3 adds compile/lint jobs but has local launch evidence on Apple Silicon only | Draft |
| REQ-NF-002 | Standalone workflows shall operate without public network access; telemetry and external upload are off and absent by default. | TEST-011 network isolation; GUI-3 capability/CSP tests are partial configuration evidence, not packet-level denial | Draft |
...
| REQ-NF-004 | Domain, geometry, runtime, and estimation engines shall be UI-independent and headlessly testable. | Architecture dependency tests; GUI-2 headless application tests and GUI-3 framework-neutral desktop contract provide partial evidence | Draft |
...
| REQ-NF-006 | The UI shall remain usable with keyboard-only input, visible focus, non-color-only states, scalable text, and documented accessibility semantics. | TEST-012 accessibility suite; GUI-3 semantic/visible-focus/keyboard smoke on Apple Silicon is partial evidence only | Draft |
...
| REQ-NF-017 | Missing, stale, conflicting, unapproved, or low-quality rate, pricing-policy, capacity, availability, PMI, requirement, CAM, and learning evidence shall be explicit and shall not silently become a neutral/default value. | TEST-002/014, TEST-047, TEST-064, TEST-068, TEST-077, TEST-082, TEST-087; GUI-3 preserves explicit not-started/session-only states | Draft |
| REQ-NF-018 | Potentially expensive optimization, simulation, revision, and reconciliation jobs shall have deterministic limits, progress, cancellation, caching policy, and resource-isolation tests. | TEST-019, TEST-043, TEST-054, TEST-059, TEST-083; Checkpoint 18 supplies partial worker isolation evidence | Draft |
