...
- **Work in progress:** TASK-003 now binds open asset roots to stable identities; evaluates explicit actor/project/record/classification/state/root context through an injected policy; defaults to versioned deny-all; appends a content-minimized decision before path resolution; fails closed on audit failure; resolves contained pathless grants; stages verified bytes in private workspaces; and claims immutable output. The 73-test authorization baseline is local pending CI; native OCCT evidence remains Apple Silicon only
- **Next task:** Add deployment policy/repository and durable append-preserving audit adapters without inventing shop roles, then controlled-store output persistence, direct worker descriptor/handle passing or a documented copy fallback, OS containment, independent fixtures, and exact native-build automation
- **Build status:** Rust 1.94.1 default workspace passes locally and on Windows/Linux/macOS in GitHub Actions run 30486941813 at commit `33ad74f`; the optional OCCT 8.0.0 native feature passes locally on Apple Silicon only
- **Test status:** TASK-002 has 41 runtime tests plus one compile-fail doctest passing locally and on three OSes; TASK-003 has 73 default runtime tests passing locally, with the preceding 66-test containment baseline passing on three OSes in run 30486941813, plus twelve focused optional-native adapter/worker tests passing locally; planning validator covers 137 documentation files
...
- **Geometry capabilities:** Kernel-neutral contracts, bounded JSON subprocess transport, a deny-by-default application authorization/audit seam, a root-identity-bound capability resolver with traversal/root-escape/final-link rejection, a consumed already-open supervisor grant, private per-job staging, immutable hash/length-bound output claiming, explicit fixture evidence, and an optional OCCT 8.0.0 worker path produce provisional STEP area/volume/centroid evidence; no deployment allow policy/repository, durable audit or controlled-store persistence, direct worker handle grant, authoritative snapshot, independent translator corpus, OS sandbox, or network/CPU/memory enforcement is implemented
