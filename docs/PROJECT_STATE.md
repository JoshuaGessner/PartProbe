...
- **Last completed task/checkpoint:** TASK-002 configurable rate mechanics are validated; TASK-003 Checkpoint 16 adds Unix direct descriptor transport with an exact exec-time allowlist, unrelated-descriptor exclusion proof, and worker-side revalidation while Windows direct HANDLE launch remains unavailable
- **Work in progress:** TASK-003 binds open roots/grants to stable identities; emits control schema v2 plus asset-transport manifest v2; selects explicit `verified_private_copy` everywhere by default or `unix_descriptor` under direct policy on Unix; independently verifies request/manifest identity, regular-file type, exact length, quota, and SHA-256 inside the worker; parses immutable verified bytes through OCCT `ReadStream`; preserves cancellation/kill/reap and deterministic cleanup; and governs claimed output. The 96-test default baseline passes locally; the prior 93-test checkpoint passes on three OSes; 21 focused Apple Silicon native tests pass locally
- **Next task:** Continue TASK-003 with a whitelist-capable Windows HANDLE launcher, OS containment/resource enforcement, independent fixtures including cancellation during a representative long transfer, and exact native-build automation; deployment policy/audit and TASK-006 durable storage remain separately evidence-gated
- **Build status:** Rust 1.94.1 default workspace passes locally at Checkpoint 16 and in prior GitHub Actions run 30702064048 at commit `f89a4d2` on Windows/Linux/macOS. Checkpoint 16 post-push CI is pending. The optional OCCT 8.0.0 ABI-v3 feature passes locally on Apple Silicon only
- **Test status:** TASK-002 has 41 runtime tests plus one compile-fail doctest passing locally and on three OSes; TASK-003 has 96 default runtime tests plus one compile-fail doctest passing locally, with its prior 93-test baseline passing on three OSes in run 30702064048, plus 21 focused optional-native adapter/worker tests passing locally; the planning validator covers 137 documentation files
...
- **Geometry capabilities:** Kernel-neutral contracts, bounded control schema v2, asset-transport manifest v2, explicit verified-private-copy selection/fallback evidence, Unix direct descriptor delivery with exact non-stdio allowlisting and unrelated-descriptor exclusion proof, fail-closed unavailable Windows direct mode, worker-side identity/type/hash/length/quota verification, immutable byte-stream adapter input, cooperative acknowledgement with forced termination, deny-by-default authorization/audit seams, root-identity-bound capability resolution, a consumed already-open grant, private per-job staging, immutable output claiming, governed storage ports, fixtures, and optional OCCT 8.0.0 ABI-v3 evidence produce provisional STEP area/volume/centroid results. STEP transfer polls cancellation; stream parsing and property calls remain internally uninterruptible. No whitelist-capable Windows HANDLE launcher, deployment allow policy/repository, durable audit/store adapter, authoritative snapshot, independent translator corpus, OS sandbox, or network/CPU/memory enforcement is implemented
