...
- **Last completed task/checkpoint:** TASK-003 now adds Linux `x86_64`/`aarch64` parser-phase syscall containment. The worker prepares irreversible `no_new_privs` before creating its cancellation reader, claims the authorized source into immutable verified bytes, then applies one `TSYNC` seccomp filter to deny socket, `io_uring`, process/thread creation, and exec syscalls. Setup failure is sanitized and fail-closed; Linux fixtures prove socket and descendant denial. The prior portable aggregate workspace-output monitor remains intact. This is narrow feature-off Linux syscall evidence, not a filesystem sandbox or configured Linux OCCT result. See `06-quality/task-003-validation.md`
- **Work in progress:** GUI-1 formal fixture/security review plus Windows/Linux native construction/runtime assembly/startup verification and remaining TASK-003 containment are the immediate parallel gates. The GUI-4/GUI-5 slice is testable but remains session-only, developer-configured, one-platform evidence with no durable shop library, save/reopen, supported importer, viewer, signed package, or production approval
- **Next task:** Verify the Linux parser filter against a pinned native OCCT runtime, extend pinned OCCT construction, schema-v1 runtime assembly/in-host verification, dynamic-link inspection, and launch evidence to Windows and Linux, then add macOS/Windows parser egress denial, general filesystem sandboxing/hard storage quotas, hard macOS memory, and hostile macOS descendant containment. In parallel, obtain GUI-1 fixture review. Then proceed through TASK-004 mesh intake, TASK-005 durable shop-owned rate/model-review UX, and TASK-006 persistence. No user input is needed for the immediate developer engineering slice; shop/security/legal review is still required at its named gates
- **Build status:** Exact OCCT `V8_0_0` commit `d3056ef80c9668f395da40f5fd7be186cae4501f` was freshly constructed on Apple Silicon and produced verified worker SHA-256 `3d3770c0cf95016800f1524f6be8fa7d6b670bac6d394b180331854ae5648482`. A 49 MiB developer runtime containing the 23-family shared-library closure verifies with manifest SHA-256 `9532bbc04e756469db3d578b95c6a59c9033327c26aeda3e4121fcea698bd2e1`; the desktop repeats verification before deriving launch paths, and the configured host smoke passes in 0.68 seconds against it. GitHub Actions run 31553615142 passes the full default code-quality matrix on macOS, Ubuntu, and Windows at Linux parser-containment implementation commit `097f78f`; this proves the feature-off Linux syscall regressions plus unchanged portable contracts, not native Windows/Linux OCCT construction/launch
- **Test status:** The current local macOS workspace passes 143 runtime tests and one compile-fail doctest, including hostile aggregate workspace-output termination/cleanup. Strict workspace/native-host/WASM Clippy, eleven Python native-tooling tests, 142-document planning validation, fixture hashes, offline frontend construction, and diff checks also pass. Run 31553615142 passes the same default gates on macOS, Ubuntu, and Windows at `097f78f`; Ubuntu additionally proves parser-phase local socket and descendant creation are denied. The existing in-host-verified GUI-5 smoke and rebuilt unsigned-app evidence remain valid but were not repeated for this feature-off worker-containment change; prior native selection, verified-runtime analysis, explicit review/input/rate/pricing submission, exact USD 702 trace, cancellation/failure/recovery evidence, and 26 optional-native adapter/worker tests remain recorded. Full screen-reader/contrast/scaling/HiDPI, configured Linux native-filter compatibility, and three-platform native/package suites remain pending
...
- **Geometry capabilities:** Kernel-neutral contracts, bounded control schema v2, asset-transport manifest v2, explicit verified-private-copy selection, exact Unix descriptor and Windows HANDLE direct delivery with unrelated-resource exclusion proof, worker-side identity/type/hash/length/quota verification, immutable byte-stream adapter input, cooperative acknowledgement with forced termination, Linux `x86_64`/`aarch64` parser socket/process syscall denial, deny-by-default authorization/audit seams, root-identity-bound capability resolution, a consumed already-open grant, private per-job staging, immutable output claiming, poll/post-exit aggregate workspace-output inspection and cleanup, governed storage ports, and a mandatory worker resource profile are implemented. GUI-4 intake can now compute a size-bounded SHA-256 fingerprint only after authorization/audit and rewind the same open grant for worker consumption; worker verification remains independent. The provisional native output has a validated schema and decoder that binds reference and source hash. General filesystem containment, hard storage quotas, hard macOS memory, macOS/Windows parser egress denial, hostile macOS descendants, configured native Linux compatibility, three-OS native construction, packaging, corpus review, and production support remain open
