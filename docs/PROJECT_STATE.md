...
- **Work in progress:** TASK-003 now consumes an already-open source grant bound to the request capability, stages verified bytes without reopening a source path, measures the synthetic STEP cube through optional OCCT 8.0.0, and rejects an invalid entity recoverably; native evidence remains Apple Silicon only
- **Next task:** Implement target-specific read-only/no-follow asset resolution and direct worker descriptor/handle passing or a documented copy fallback, then add OS containment, independent fixtures, and exact native-build automation
- **Build status:** Rust 1.94.1 default workspace passes locally and on Windows/Linux/macOS in GitHub Actions run 30477805222 at commit `96f3bf8`; the optional OCCT 8.0.0 native feature passes locally on Apple Silicon only
- **Test status:** TASK-002 has 41 runtime tests plus one compile-fail doctest passing locally and on three OSes; TASK-003 has 63 default runtime tests passing in run 30477805222 plus twelve focused optional-native adapter/worker tests passing locally; planning validator covers 137 documentation files
...
- **Geometry capabilities:** Kernel-neutral contracts, bounded JSON subprocess transport, a consumed already-open supervisor grant with fixed-name verified staging, explicit fixture evidence, and an optional OCCT 8.0.0 worker path produce provisional STEP area/volume/centroid evidence; no application resolver no-follow policy, direct worker handle grant, authoritative snapshot, independent translator corpus, OS sandbox, or network/CPU/memory enforcement is implemented
