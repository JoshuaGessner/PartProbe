...
- **Work in progress:** TASK-003 now resolves normalized relative local sources beneath an open directory capability without traversal, root escape, or final-link following; runs each grant in a private job directory; stages verified bytes without reopening a source path; claims referenced output as immutable hash/length-bound bytes; measures the synthetic STEP cube through optional OCCT 8.0.0; and rejects an invalid entity recoverably. Capability-root evidence passes on three OSes; native evidence remains Apple Silicon only
- **Next task:** Add the application/security-service authorization and audit decision point for actor, project, classification, record state, approved root, and asset capability; then direct worker descriptor/handle passing or a documented copy fallback, controlled-store output persistence, OS containment, independent fixtures, and exact native-build automation
...
- **Test status:** TASK-002 has 41 runtime tests plus one compile-fail doctest passing locally and on three OSes; TASK-003 has 66 default runtime tests passing locally and on three OSes in run 30486941813 plus twelve focused optional-native adapter/worker tests passing locally; planning validator covers 137 documentation files
...
- **Geometry capabilities:** Kernel-neutral contracts, bounded JSON subprocess transport, a capability-root relative resolver with traversal/root-escape/final-link rejection, a consumed already-open supervisor grant, private per-job staging, immutable hash/length-bound output claiming, explicit fixture evidence, and an optional OCCT 8.0.0 worker path produce provisional STEP area/volume/centroid evidence; no application-service actor/project/classification authorization or audit, direct worker handle grant, controlled-store persistence, authoritative snapshot, independent translator corpus, OS sandbox, or network/CPU/memory enforcement is implemented
