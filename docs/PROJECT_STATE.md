...
- **Last completed task/checkpoint:** TASK-003 developer-runtime startup enforcement and its actual-app repeat are complete for the bounded Apple-Silicon path. A platform-neutral Rust verifier repeats the schema-v1 source/build/host/artifact/closure checks inside the desktop host, derives the worker/native-library paths only after success, and rejects independent overrides; five verifier tests, the opt-in real STEP-to-estimate smoke, and the rebuilt unsigned app's complete verified-runtime STEP-to-USD-702 flow pass. See `06-quality/task-003-validation.md` and `06-quality/gui-5-validation.md`
