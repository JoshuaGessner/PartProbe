...
- **Build status:** GUI-3 focused tests, workspace/native-host/WASM strict Clippy, release Trunk bundle, and a freshly rebuilt unsigned Apple-Silicon `.app` pass locally. Run 31327747010 passes macOS and Ubuntu; Windows passes every lint/build precursor but reaches the CPU fixture's former 5-second wall deadline before accumulating its configured 100 ms user-CPU ceiling. The approved test-only fix gives that fixture 30 seconds while preserving the same hard CPU ceiling and expected Job-limit exit; replacement three-OS evidence is pending push
