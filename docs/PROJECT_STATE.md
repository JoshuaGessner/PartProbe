...
- **Last completed task/checkpoint:** TASK-003 now adds portable, poll-bounded aggregate private-workspace output containment. The supervisor preserves the staged-copy size/read-only invariant, rejects links/special/nested entries, caps entries, sums every non-input regular file against the request output quota during execution and after exit, terminates a live violation, and recursively removes only its owned job directory. A hostile two-file fixture proves aggregate termination and scratch cleanup locally. This remains supervisory containment, not a hard filesystem quota or sandbox. See `06-quality/task-003-validation.md`
...
- **Next task:** Extend pinned OCCT construction, schema-v1 runtime assembly/in-host verification, dynamic-link inspection, and launch evidence to Windows and Linux while closing worker network/filesystem sandboxing, hard storage quotas, hard macOS memory, and hostile Unix descendant containment. In parallel, obtain GUI-1 fixture review. Then proceed through TASK-004 mesh intake, TASK-005 durable shop-owned rate/model-review UX, and TASK-006 persistence. No user input is needed for the immediate developer engineering slice; shop/security/legal review is still required at its named gates
- **Build status:** Exact OCCT `V8_0_0` commit `d3056ef80c9668f395da40f5fd7be186cae4501f` was freshly constructed on Apple Silicon and produced verified worker SHA-256 `3d3770c0cf95016800f1524f6be8fa7d6b670bac6d394b180331854ae5648482`. A 49 MiB developer runtime containing the 23-family shared-library closure verifies with manifest SHA-256 `9532bbc04e756469db3d578b95c6a59c9033327c26aeda3e4121fcea698bd2e1`; the desktop now repeats verification before deriving launch paths, and the configured host smoke passes in 0.68 seconds against it. GitHub Actions run 31551572742 passes the full default code-quality matrix on macOS, Ubuntu, and Windows at aggregate workspace-output implementation commit `7b23e51`; this proves portable feature-off contracts and supervisor monitoring, not native Windows/Linux OCCT construction/launch
- **Test status:** The current local macOS workspace passes 143 runtime tests and one compile-fail doctest, including the new hostile aggregate workspace-output case. The hostile case terminates in under two seconds and leaves its dedicated controlled parent empty. Strict workspace/native-host/WASM Clippy, eleven Python native-tooling tests, 142-document planning validation, fixture hashes, offline frontend construction, and diff checks also pass. Run 31551572742 passes the same default gates on macOS, Ubuntu, and Windows at `7b23e51`, including the hostile aggregate-output regression. The existing in-host-verified GUI-5 smoke and rebuilt unsigned-app evidence remain valid but were not repeated for this worker-supervisor-only change; prior native selection, verified-runtime analysis, explicit review/input/rate/pricing submission, exact USD 702 trace, cancellation/failure/recovery evidence, and 26 optional-native adapter/worker tests remain recorded. Full screen-reader/contrast/scaling/HiDPI and three-platform native/package suites remain pending
...
- **Geometry capabilities:** Kernel-neutral contracts, bounded control schema v2, asset-transport manifest v2, explicit verified-private-copy selection, exact Unix descriptor and Windows HANDLE direct delivery with unrelated-resource exclusion proof, worker-side identity/type/hash/length/quota verification, immutable byte-stream adapter input, cooperative acknowledgement with forced termination, deny-by-default authorization/audit seams, root-identity-bound capability resolution, a consumed already-open grant, private per-job staging, immutable output claiming, poll/post-exit aggregate workspace-output inspection and cleanup, governed storage ports, and a mandatory worker resource profile are implemented. GUI-4 intake can now compute a size-bounded SHA-256 fingerprint only after authorization/audit and rewind the same open grant for worker consumption; worker verification remains independent. The provisional native output has a validated schema and decoder that binds reference and source hash. OS-enforced network/filesystem containment, hard storage quotas, hard macOS memory, hostile Unix descendant containment, three-OS native construction, packaging, corpus review, and production support remain open
