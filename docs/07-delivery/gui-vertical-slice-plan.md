...
GUI-4 is **In Progress**. The application boundary now accepts a validated path-free request template, authorizes and audits the selected relative source, computes its bounded SHA-256 fingerprint from that same already-open grant, constructs the source-bound worker request, rewinds the grant, and passes it to the geometry port for independent worker verification. This closes the unsafe integration temptation to hash CAD in the Tauri host before authorization. Native runtime configuration, desktop commands/view models, analysis/review/input/rate forms, deterministic result rendering, and cancellation remain open; see [GUI-4 validation evidence](../06-quality/gui-4-validation.md).
...
| GUI-4 — Analysis and estimate workspace | **In Progress.** Governed post-authorization source fingerprint/request construction is implemented; import/progress/cancel/error states, unit confirmation, geometry facts/warnings, editable manual estimate inputs/rates, quantity and itemized cost/price trace remain | An estimator can choose the fixture, review evidence, supply every required input, and obtain a deterministic provisional estimate; unavailable/conflicting values remain visibly blocked |
