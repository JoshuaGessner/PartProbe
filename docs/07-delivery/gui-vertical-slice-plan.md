...
GUI-2 is **In Review** with its implementation and focused local acceptance tests passing. `DraftEstimateApplication` authorizes and audits the local source before handing a consumed pathless grant to a geometry-analysis port; the production port is implemented by `GeometryWorkerSupervisor`. The resulting ephemeral session retains provisional worker evidence, requires explicit unit/warning review, accepts complete manual stock/material/time/cost/quantity inputs, resolves five approved hourly rate categories from a pinned card/date/scope context, applies a pinned pricing policy through the estimation engine, and returns an itemized replay trace. Four headless integration tests cover deny-before-analysis, missing and conflicting rate states, deterministic golden reconciliation, trace pinning, and input-edit recalculation. See [GUI-2 validation evidence](../06-quality/gui-2-validation.md).
...
| GUI-2 — Application use case | **Implemented; In Review.** One session-scoped draft-estimate service composes authorized intake, worker supervision, unit/warning review, geometry facts, manual stock/material/setup/run/quantity inputs, rate resolution, and pricing | Four headless integration tests prove blocked missing data, deterministic recalculation, traceability, authorization ordering, and no UI-owned calculation; exact-head CI/review closes the checkpoint |
...
GUI-1 still awaits fixture review, while GUI-2 implementation is ready for exact-head review. Approximately three focused implementation checkpoints remain for the facts-and-estimate GUI: the secure desktop shell, analysis/estimate workspace, and end-to-end smoke evidence. GUI-3 is now the code critical path; GUI-1 fixture review remains a parallel evidence gate.
