...
A restrictive Tauri/Leptos developer shell, typed desktop contract, native STEP/STL/3MF picker, and unsigned local macOS bundle now exist. Contract v4 keeps the same five-command boundary, connects a retained selection token to the completed estimate-workflow application service, provides token-bound cooperative cancellation, and returns tagged path-free exact-B-rep or mesh evidence. Exact STEP can submit explicit review/manual/rate/pricing inputs to the retained native session for a deterministic result trace. Mesh results preserve unresolved versus canonical units, optional measurements, confidence, topology, welding, and warnings, but the application keeps estimating unavailable and the frontend withholds the form. TASK-003 can assemble a content-addressed developer runtime containing the verified worker and full OCCT shared-library closure, independently of mutable Cargo output; Apple Silicon passes the actual STEP app flow, while Ubuntu x86_64 passes construction, link auditing, the configured host smoke, and an extracted unsigned Debian package whose fixed resource child contains and re-verifies that runtime. The Apple bundle still uses an explicit external developer runtime, and neither result is signed, installed, legally approved for distribution, or cross-platform product packaging. Configured native runs 33546682993 and 33546698680 drive governed STEP/STL/3MF through the retained host boundary on Ubuntu and Windows at exact commit `7f2c378`. No model viewer, tessellation contract, durable repository, report renderer, windowed cross-platform mesh acceptance, or supported production importer exists yet. Default feature-off exact geometry builds intentionally report `NATIVE_ADAPTER_UNAVAILABLE`; bounded STL/3MF worker parsing does not establish product support.
...
GUI-4 is **Complete for its bounded exact-STEP workspace checkpoint**. The application boundary performs post-authorization source fingerprint/request construction, and desktop contract v4 connected the retained selection token to an explicitly configured supervised worker without changing the then-current five-command manifest. The native adapter applies a developer-session policy/audit, retains `DraftEstimateSession`, owns cooperative cancellation, constructs typed session-only rate/pricing contexts from confirmed exact text, and returns only path-free geometry/result traces. Leptos renders running/cancelling/cancelled/failure states, revision-bound exact review and complete no-default inputs, blocked/unavailable results, deterministic exact itemized output, and non-authoritative mesh facts with explicit withheld measurements and estimate blocking. GUI-5 supplied the configured STEP runtime and application evidence; configured Linux/Windows native-host mesh evidence now passes, while windowed mesh interaction remains open. USE-2 contracts v5-v7 produce seven analysis/Settings commands, contract v8 adds one native-evidence catalog activation request, and contract v9 adds one complete bounded native-evidence catalog-draft save request. Ordinary startup returns identity unavailability for both catalog commands and Leptos invokes neither. None of these changes alter GUI-4's completion boundary. See [GUI-4 validation evidence](../06-quality/gui-4-validation.md).

GUI-5 is **Complete for the Apple-Silicon internal developer checkpoint**. A freshly constructed pinned OCCT 8.0.0 worker passed an opt-in real-host smoke and the actual unsigned application completed the fixed prism workflow by keyboard, displayed the expected geometry and USD 702 trace, exposed semantic input/result structure, handled live cancellation distinctly, failed safely on malformed STEP, recovered on a subsequent valid source, and cleared review confirmations on re-analysis. The same smoke now also passes against a separately assembled and verified developer runtime, eliminating dependence on the shared Cargo worker output for subsequent testing. This is one-platform synthetic/session-only evidence, not a supported product capability; see [GUI-5 validation evidence](../06-quality/gui-5-validation.md) and [TASK-003 validation evidence](../06-quality/task-003-validation.md).
