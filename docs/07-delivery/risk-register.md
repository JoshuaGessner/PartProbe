...
|---|---|---|---|---|---|---|---|---|---|
| RISK-001 | OCCT/FFI defect, malformed CAD, wrong-resource binding, or unintended descriptor/HANDLE inheritance escapes/crashes/leaks from the worker boundary | M | Critical | Isolated worker, identity-bound verified-copy transport, immutable byte-stream parsing, quotas, exact Unix descriptor and Windows HANDLE allowlists with sentinel proof, corpus/fuzzing, OS sandbox/resource limits, restart boundaries | Geometry lead | Host crash, escape, unrelated inherited resource, fallback represented as direct, leak, or corrupted output | 2026-08-01 | M0.2–M2 | Open |
