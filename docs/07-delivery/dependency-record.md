...
This record covers dependencies introduced by TASK-001. Exact versions are pinned in the workspace manifest and lockfile. Presence here records engineering review; it is not legal advice or a final organization-wide license approval.
...
| `serde_json` | 1.0.151; defaults | Compact canonical snapshot representation for the reversible spike | custom canonical encoder; another canonical binary/text format | MIT OR Apache-2.0; crates.io/upstream GitHub | Owner: calculation maintainers; replace through `CalculationSnapshot::to_canonical_json` before formats become external contracts |
