...
- Project crates retain `unsafe_code = "forbid"` except `platform`, which sets `unsafe_code = "deny"` and permits only two documented functions: the stable Unix `pre_exec` hook and conversion of one validated inherited raw descriptor into its unique Rust owner. This does not prove that dependencies contain no unsafe code; automated unsafe/advisory review remains a release gate.
