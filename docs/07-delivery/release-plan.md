...
The release-critical sequence is now organized by the [usable estimator delivery plan](usable-estimator-plan.md): finish the USE-1 upload-first workflow guardrail; build USE-2 durable governed shop Settings; add USE-3 exact-STEP stock/material proposals; add USE-4 coarse process/runtime proposals; then validate the streamlined USE-5 model-to-estimate workflow and USE-6 calibration/deployment gates. TASK-003 worker/native and TASK-004 format evidence continue as required safety/support tracks, but repeated format hardening does not substitute for the missing estimate-input derivation layers. Advanced routing, uncertainty, PMI, learning, CAM, capacity, and integration features remain outside the initial release gate.
