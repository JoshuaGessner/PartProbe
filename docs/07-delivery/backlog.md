...
TASK-003 is **In Progress** on 2026-08-01. Its stacked slices add kernel-neutral contracts, bounded control schema v2, asset-transport manifest v2, explicit `verified_private_copy`, Unix `unix_descriptor` and Windows `windows_handle` with exact child-resource allowlists and unrelated-resource exclusion proof, worker-side identity/type/hash/length/quota verification, cooperative cancellation with forced termination, a mandatory pre-execution resource profile, Unix CPU/file/process-group controls, Linux memory limits, suspended Windows CPU/memory/one-process Job containment, fixture expectation schema v2, deterministic synthetic STEP generation, optional OCCT 8.0.0 ABI v3 parsing the exact verified bytes while polling cancellation during STEP root transfer, and a validated provisional snapshot/reference/source-binding seam with a native-root fingerprint/build/test entry point. Checkpoint 18 passes three-OS default CI; Checkpoint 19 passes locally and awaits exact-head three-OS CI. Representative mid-transfer cancellation, internally uninterruptible stream/property bounds, independent STEP fixtures, network/filesystem and remaining resource containment, legal approval, reproducible OCCT construction/packaging, and three-OS native benchmarks remain required; see [TASK-003 validation](../06-quality/task-003-validation.md).

The [testable GUI vertical-slice plan](gui-vertical-slice-plan.md) defines a smaller internal, STEP-only, session-only developer path that can begin after its native test seam is reproducible. Passing that path does not complete TASK-003, TASK-005, TASK-006, M1, or M2.
