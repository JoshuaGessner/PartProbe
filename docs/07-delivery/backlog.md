...
TASK-004 is **In Progress** on 2026-08-24. Its bounded comparison slices add path-free ASCII/binary STL plus in-memory ZIP/OPC 3MF analysis over thirteen successful governed mesh fixtures and twenty persisted rejected mesh inputs. STL keeps units unresolved; successful 3MF packages cover all Core units/default, direct through two-link linear component chains, ordered transforms, canonical conversion, and count-only model metadata. Eighteen rejected 3MF packages pin exact no-evidence diagnostics for selected graph/reference/union/metadata/material/extension/archive threats, including encryption and unsafe absolute/backslash/directory entry names; two rejected binary STL inputs pin truncated framing and unsupported facet attributes. All accepted formats share triangle/bounds/area/manifold/watertight/winding and conditional closed-volume/centroid measurement. The corpus derives from one analytic cube, not independent representative geometry. Supported binary attribute/color semantics, general DAG/material semantics, broader malformed numeric/resource-limit cases, self-intersection/confidence policy, worker/desktop integration, representative fixtures, and production support remain open; see [TASK-004 validation](../06-quality/task-004-validation.md).
...
| TASK-004 | **In Progress:** implement STL/3MF mesh import comparison spike; bounded ASCII/binary STL, direct/linear-component-chain/model-metadata 3MF success evidence, and twenty typed mesh rejection fixtures pass | REQ-F-002–004; ADR-0004; TEST-003 | Unit/mesh validity measurements and failure matrix |
