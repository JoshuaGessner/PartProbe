...
TASK-003 is **In Progress** on 2026-08-22. Its stacked slices add kernel-neutral contracts, bounded control/transport schemas, exact verified-copy/Unix-descriptor/Windows-HANDLE delivery, worker byte verification, cancellation, partial target resource containment, fixture expectation schema v3, optional OCCT 8.0.0 ABI-v3 parsing, a source-bound provisional snapshot, fail-closed schema-v1 runtime assembly, aggregate-workspace monitoring, Linux parser socket/process syscall denial, and Ubuntu/Windows configured-native evidence. Windows run 31613259785 passes exact Visual Studio 2022/x64 construction, all 27 native cases, app-local runtime assembly/verification, 24-binary/23-family x64 PE closure, and configured STEP-to-USD-702 host smoke. Full Linux package run 32610311910 passes the fixed packaged-resource contract with an extracted unsigned Debian payload, repeated runtime/link verification, real STEP-to-USD-702 analysis, bounded launch, real portal selection, and exact application-owned source/geometry state on one virtual configuration. Representative cancellation, formal fixture/corpus review, macOS/Windows parser egress denial, filesystem sandboxing/hard quotas, legal approval, broader Linux/interactive Windows evidence, signed three-OS GUI packaging, and three-OS native benchmarks remain required; see [TASK-003 validation](../06-quality/task-003-validation.md).

TASK-004 is **In Progress** on 2026-08-26. Its bounded comparison slices add path-free ASCII/binary STL plus in-memory ZIP/OPC 3MF analysis over seventeen successful governed mesh fixtures and twenty-nine persisted rejected mesh inputs. STL keeps units unresolved; successful 3MF packages cover all Core units/default, direct through two-link linear component chains, ordered transforms, canonical conversion, count-only model metadata, and three isolated legal OPC variations. Versioned exact-predicate intersection evidence and categorical reason-coded confidence cap valid mesh at `Low`; unresolved units, topology defects, detected intersection, or indeterminate coplanar overlap require `NeedsReview`, and intersection uncertainty withholds enclosed volume/centroid. `FIX-MESH-039` persists intersecting closed tetrahedral shells. Twenty-one rejected 3MF packages pin exact no-evidence diagnostics for selected graph/reference/union/metadata/material/extension/archive/XML/resource threats; four rejected binary STL inputs pin truncated framing, unsupported facet attributes, non-finite numeric data, and triangle-count overflow; and four rejected ASCII STL inputs pin invalid UTF-8, malformed facet grammar, empty-solid, and degenerate-triangle behavior. Most format fixtures derive from one analytic cube and the tetrahedra remain project-authored rather than independent representative geometry. Supported binary attribute/color semantics, general DAG/material semantics, broader exporter/legal-package and representative malformed entity/geometry cases, production welding/tolerance policy, worker/desktop integration, representative fixtures, and production support remain open; see [TASK-004 validation](../06-quality/task-004-validation.md).
