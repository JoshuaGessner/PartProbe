...
TASK-003 is **In Progress** on 2026-08-12. Its stacked slices add kernel-neutral contracts, bounded control/transport schemas, exact verified-copy/Unix-descriptor/Windows-HANDLE delivery, worker byte verification, cancellation, partial target resource containment, fixture expectation schema v2, optional OCCT 8.0.0 ABI-v3 parsing, a source-bound provisional snapshot, fail-closed schema-v1 runtime assembly, aggregate-workspace monitoring, Linux parser socket/process syscall denial, and Ubuntu/Windows configured-native evidence. Windows run 31613259785 passes exact Visual Studio 2022/x64 construction, all 27 native cases, app-local runtime assembly/verification, 24-binary/23-family x64 PE closure, and configured STEP-to-USD-702 host smoke. The desktop host now checks one fixed packaged-resource runtime location, and an opt-in combined Linux native-package gate is ready but has not yet produced hosted evidence. Representative cancellation, formal fixture/corpus review, macOS/Windows parser egress denial, filesystem sandboxing/hard quotas, legal approval, passing native-runtime-integrated GUI packaging, signed integration, and three-OS native benchmarks remain required; see [TASK-003 validation](../06-quality/task-003-validation.md).
