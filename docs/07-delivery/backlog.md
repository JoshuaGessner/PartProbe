...
TASK-004 is **In Progress** on 2026-08-22. Its first bounded comparison slice adds a path-free, byte/triangle-quota-limited ASCII STL analyzer with explicit unresolved units, triangle/bounds/area evidence, manifold/watertight/winding classification, conditional closed volume/centroid, sanitized failures, and golden regressions for the governed closed/open synthetic cubes. Binary STL, 3MF, self-intersection/confidence policy, desktop/worker integration, representative fixtures, and production support remain open; see [TASK-004 validation](../06-quality/task-004-validation.md).
...
| TASK-004 | **In Progress:** implement STL/3MF mesh import comparison spike; bounded ASCII STL fixture evidence passes | REQ-F-002–004; ADR-0004; TEST-003 | Unit/mesh validity measurements and failure matrix |
