...
TASK-004 is **In Progress** on 2026-08-22. Its first six bounded comparison slices add path-free ASCII/binary STL plus in-memory ZIP/OPC 3MF analysis over twelve governed fixtures. STL keeps units unresolved; nine persisted 3MF packages cover all six Core unit declarations/default, direct or one single-reference component object, component-then-build transform order, canonical conversion, and count-only well-known model-metadata presence under archive/XML/entity/object/component/metadata/compression ceilings. All formats share triangle/bounds/area/manifold/watertight/winding and conditional closed-volume/centroid measurement. The corpus is multiple encodings of one analytic cube, not independent representative geometry. Binary attribute/color extensions, broader component graphs/unions, object/item/vendor metadata, adversarial layouts, self-intersection/confidence policy, worker/desktop integration, representative fixtures, and production support remain open; see [TASK-004 validation](../06-quality/task-004-validation.md).
...
| TASK-004 | **In Progress:** implement STL/3MF mesh import comparison spike; bounded ASCII/binary STL and direct/single-component/model-metadata 3MF fixture evidence passes | REQ-F-002–004; ADR-0004; TEST-003 | Unit/mesh validity measurements and failure matrix |
