...
TASK-004 is **In Progress** on 2026-08-22. Its first four bounded comparison slices add path-free ASCII/binary STL plus in-memory ZIP/OPC 3MF analysis over five governed fixtures. STL keeps units unresolved; 3MF validates its model relationship/content type, explicit/default units, direct or one single-reference component object, component-then-build transform order, canonical conversion, and path-free provenance under archive/XML/entity/object/component/compression ceilings. All formats share triangle/bounds/area/manifold/watertight/winding and conditional closed-volume/centroid measurement. Binary attribute/color extensions, persisted per-unit 3MF fixtures, broader component graphs/unions, metadata/adversarial layouts, self-intersection/confidence policy, worker/desktop integration, representative fixtures, and production support remain open; see [TASK-004 validation](../06-quality/task-004-validation.md).

The [testable GUI vertical-slice plan](gui-vertical-slice-plan.md) defines a smaller internal, STEP-only, session-only developer path. GUI-1 implementation is awaiting fixture review; GUI-2 through GUI-5 are complete for their bounded checkpoint scopes, including actual Apple-Silicon STEP-to-estimate evidence. Ubuntu 22.04 run 31624958771 adds unsigned extracted Debian package-resource verification, native STEP analysis, and bounded payload launch. Short run 32607101403 passes real XDG-portal prism selection through the exact feature-off Debian payload without path or coordinate bypass. Full run 32607742609 passes every native/package/headless-analysis and exact-selection step before a superseded visible-text AT-SPI assertion; the local exact geometry-label correction awaits a new hosted run. Installation, signing, support, full-form completion, and broad accessibility remain absent. AppImage is deferred until its helper inputs are pinned. Passing either path does not complete TASK-003, TASK-005, TASK-006, M1, or M2.
