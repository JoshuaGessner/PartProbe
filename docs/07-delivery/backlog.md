...
TASK-004 is **In Progress** on 2026-08-26. Its bounded comparison slices add path-free ASCII/binary STL plus in-memory ZIP/OPC 3MF analysis over twenty successful governed mesh fixtures and twenty-nine persisted rejected mesh inputs. STL keeps units unresolved and now persists separate open-boundary, reversed-facet, non-manifold-edge, detected-intersection, and coplanar-indeterminate warning outcomes with closed measurements withheld when unauthorized. Successful expectation schema v4 preserves all three detector states rather than a boolean. Successful 3MF packages cover all Core units/default, direct through two-link linear component chains, ordered transforms, canonical conversion, count-only model metadata, and three isolated legal OPC variations. Versioned exact-predicate intersection evidence and categorical reason-coded confidence cap valid mesh at `Low`; unresolved units, topology defects, detected intersection, or indeterminate coplanar overlap require `NeedsReview`. Twenty-one rejected 3MF packages pin exact no-evidence diagnostics for selected graph/reference/union/metadata/material/extension/archive/XML/resource threats; four rejected binary STL inputs pin truncated framing, unsupported facet attributes, non-finite numeric data, and triangle-count overflow; and four rejected ASCII STL inputs pin invalid UTF-8, malformed facet grammar, empty-solid, and degenerate-triangle behavior. Most format fixtures derive from one analytic cube or project-authored tetrahedra rather than independent representative geometry. Supported binary attribute/color semantics, general DAG/material semantics, broader exporter/legal-package and representative malformed entity/geometry cases, production welding/tolerance policy, worker/desktop integration, representative fixtures, and production support remain open; see [TASK-004 validation](../06-quality/task-004-validation.md).
