...
TASK-004 is **In Progress** on 2026-08-22. Its first three bounded comparison slices add path-free ASCII/binary STL plus in-memory ZIP/OPC 3MF analysis over four governed fixtures. STL keeps units unresolved; 3MF validates its model relationship/content type, explicit/default units, one direct mesh/build item, canonical conversion, and build transform under archive/XML/entity/compression ceilings. All formats share triangle/bounds/area/manifold/watertight/winding and conditional closed-volume/centroid measurement. Binary attribute/color extensions, broader 3MF units/components/metadata/adversarial layouts, self-intersection/confidence policy, worker/desktop integration, representative fixtures, and production support remain open; see [TASK-004 validation](../06-quality/task-004-validation.md).
...
| TASK-004 | **In Progress:** implement STL/3MF mesh import comparison spike; bounded ASCII/binary STL and direct-mesh 3MF fixture evidence passes | REQ-F-002–004; ADR-0004; TEST-003 | Unit/mesh validity measurements and failure matrix |
