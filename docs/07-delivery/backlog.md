...
TASK-003 is **In Progress** on 2026-08-22. Its stacked slices add kernel-neutral contracts, bounded control/transport schemas, exact verified-copy/Unix-descriptor/Windows-HANDLE delivery, worker byte verification, cancellation, partial target resource containment, fixture expectation schema v2, optional OCCT 8.0.0 ABI-v3 parsing, a source-bound provisional snapshot, fail-closed schema-v1 runtime assembly, aggregate-workspace monitoring, Linux parser socket/process syscall denial, and Ubuntu/Windows configured-native evidence. Windows run 31613259785 passes exact Visual Studio 2022/x64 construction, all 27 native cases, app-local runtime assembly/verification, 24-binary/23-family x64 PE closure, and configured STEP-to-USD-702 host smoke. Full Linux package run 32610311910 passes the fixed packaged-resource contract with an extracted unsigned Debian payload, repeated runtime/link verification, real STEP-to-USD-702 analysis, bounded launch, real portal selection, and exact application-owned source/geometry state on one virtual configuration. Representative cancellation, formal fixture/corpus review, macOS/Windows parser egress denial, filesystem sandboxing/hard quotas, legal approval, broader Linux/interactive Windows evidence, signed three-OS GUI packaging, and three-OS native benchmarks remain required; see [TASK-003 validation](../06-quality/task-003-validation.md).

TASK-004 is **In Progress** on 2026-08-24. Its bounded comparison slices add path-free ASCII/binary STL plus in-memory ZIP/OPC 3MF analysis over thirteen successful governed mesh fixtures and twenty-five persisted rejected mesh inputs. STL keeps units unresolved; successful 3MF packages cover all Core units/default, direct through two-link linear component chains, ordered transforms, canonical conversion, and count-only model metadata. Twenty-one rejected 3MF packages pin exact no-evidence diagnostics for selected graph/reference/union/metadata/material/extension/archive/XML/resource threats, including encryption, unsafe entry names, malformed XML, forbidden document type, and entry-count overflow; four rejected binary STL inputs pin truncated framing, unsupported facet attributes, non-finite numeric data, and triangle-count overflow. All accepted formats share triangle/bounds/area/manifold/watertight/winding and conditional closed-volume/centroid measurement. The corpus derives from one analytic cube, not independent representative geometry. Supported binary attribute/color semantics, general DAG/material semantics, broader malformed entity/geometry cases, self-intersection/confidence policy, worker/desktop integration, representative fixtures, and production support remain open; see [TASK-004 validation](../06-quality/task-004-validation.md).
...
| TASK-004 | **In Progress:** implement STL/3MF mesh import comparison spike; bounded ASCII/binary STL, direct/linear-component-chain/model-metadata 3MF success evidence, and twenty-five typed mesh rejection fixtures pass | REQ-F-002–004; ADR-0004; TEST-003 | Unit/mesh validity measurements and failure matrix |
