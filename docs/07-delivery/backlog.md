...
TASK-004 is **In Progress** on 2026-08-24. Its bounded comparison slices add path-free ASCII/binary STL plus in-memory ZIP/OPC 3MF analysis over fourteen successful governed mesh fixtures and twenty-nine persisted rejected mesh inputs. STL keeps units unresolved; successful 3MF packages cover all Core units/default, direct through two-link linear component chains, ordered transforms, canonical conversion, and count-only model metadata. Versioned exact-predicate intersection evidence and categorical reason-coded confidence cap valid mesh at `Low`; unresolved units, topology defects, detected intersection, or indeterminate coplanar overlap require `NeedsReview`, and intersection uncertainty withholds enclosed volume/centroid. `FIX-MESH-039` persists intersecting closed tetrahedral shells. Twenty-one rejected 3MF packages pin exact no-evidence diagnostics for selected graph/reference/union/metadata/material/extension/archive/XML/resource threats; four rejected binary STL inputs pin truncated framing, unsupported facet attributes, non-finite numeric data, and triangle-count overflow; and four rejected ASCII STL inputs pin invalid UTF-8, malformed facet grammar, empty-solid, and degenerate-triangle behavior. Most format fixtures derive from one analytic cube and the tetrahedra remain project-authored rather than independent representative geometry. Supported binary attribute/color semantics, general DAG/material semantics, broader representative malformed entity/geometry cases, production welding/tolerance policy, worker/desktop integration, representative fixtures, and production support remain open; see [TASK-004 validation](../06-quality/task-004-validation.md).
