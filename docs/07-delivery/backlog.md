...
TASK-003 is **In Progress** on 2026-08-22. Its stacked slices add kernel-neutral contracts, bounded control/transport schemas, exact verified-copy/Unix-descriptor/Windows-HANDLE delivery, worker byte verification, cancellation, partial target resource containment, current successful-fixture expectation schema v4, optional OCCT 8.0.0 ABI-v3 parsing, a source-bound provisional snapshot, fail-closed schema-v1 runtime assembly, aggregate-workspace monitoring, Linux parser socket/process syscall denial, and Ubuntu/Windows configured-native evidence. Windows run 31613259785 passes exact Visual Studio 2022/x64 construction, all 27 native cases, app-local runtime assembly/verification, 24-binary/23-family x64 PE closure, and configured STEP-to-USD-702 host smoke. Full Linux package run 32610311910 passes the fixed packaged-resource contract with an extracted unsigned Debian payload, repeated runtime/link verification, real STEP-to-USD-702 analysis, bounded launch, real portal selection, and exact application-owned source/geometry state on one virtual configuration. Representative cancellation, formal fixture/corpus review, macOS/Windows parser egress denial, filesystem sandboxing/hard quotas, legal approval, broader Linux/interactive Windows evidence, signed three-OS GUI packaging, and three-OS native benchmarks remain required; see [TASK-003 validation](../06-quality/task-003-validation.md).

TASK-004 is **In Progress** on 2026-08-31. Its bounded comparison slices add path-free ASCII/binary STL plus in-memory ZIP/OPC 3MF analysis over twenty-two successful governed mesh fixtures and thirty-seven persisted rejected mesh inputs. Successful governed STL/3MF bytes now also cross the isolated worker through verified transport, immutable-byte content dispatch, additive source-bound `geometry-mesh-snapshot-v1`, controlled-output claiming, and cleanup while exact STEP retains `geometry-snapshot-v1`. STL keeps units unresolved and persists separate open-boundary, reversed-facet, non-manifold-edge, detected-intersection, and coplanar-indeterminate warning outcomes with closed measurements withheld when unauthorized. Successful expectation schema v4 preserves all three detector states rather than a boolean. Successful 3MF packages cover all Core units/default, direct through two-link linear component chains, ordered transforms, canonical conversion, count-only model metadata, and four isolated legal OPC variations including a standard internal package thumbnail. Versioned exact-predicate intersection evidence and categorical reason-coded confidence cap valid mesh at `Low`; unresolved units, topology defects, detected intersection, or indeterminate coplanar overlap require `NeedsReview`. Twenty-nine rejected 3MF packages pin exact no-evidence diagnostics for selected graph/reference/union/metadata/material/extension/archive/XML/resource threats, the unsupported singular/reflected-transform policy boundary, incomplete/non-finite vertex data, repeated/out-of-range triangle indices, an empty triangle structure, and the configured vertex-count ceiling. Negative-determinant transforms remain a deferred legal Core behavior requiring correct orientation/volume-sign handling, not malformed input. Four rejected binary STL inputs and four rejected ASCII STL inputs pin selected framing/attribute/numeric/grammar/empty/degenerate failures. Most format fixtures derive from one analytic cube or project-authored tetrahedra rather than independent representative geometry. Supported binary attribute/color semantics, negative-determinant interpretation, general DAG/material semantics, broader exporter/legal-package and representative malformed entity/geometry cases, production welding/tolerance policy, application/desktop mesh presentation, representative fixtures, and production support remain open; see [TASK-004 validation](../06-quality/task-004-validation.md).

The [testable GUI vertical-slice plan](gui-vertical-slice-plan.md) defines a smaller internal, STEP-only, session-only developer path. GUI-1 implementation is awaiting fixture review; GUI-2 through GUI-5 are complete for their bounded checkpoint scopes, including actual Apple-Silicon STEP-to-estimate evidence. Full Ubuntu 22.04 run 32610311910 adds unsigned extracted Debian package-resource verification, native STEP analysis, bounded payload launch, real portal selection, and exact application-owned source/geometry evidence on one virtual configuration. Installation, signing, support, full-form completion, and broad accessibility remain absent. AppImage is deferred until its helper inputs are pinned. Passing either path does not complete TASK-003, TASK-005, TASK-006, M1, or M2.
