...
| VIS-2 governed display derivative — In Progress | The native-only manifest/framing/decoder bind source and analysis identity, profile, bounds, layouts, references, chunks, hashes, and limits. Additive OCCT display ABI v1 tessellates the same immutable STEP bytes after measurement, worker schema v2 emits one source/analysis-bound artifact, the supervisor claims both fixed outputs under one quota, and the application retains only validated arrays plus diagnostics | Complete: manifest/artifact rejection suites, protocol migration, shared-quota dual claim, native cube/prism tessellation/emission, hard limit/cancellation, and real supervisor/application retention on Apple Silicon. Open: desktop activation, renderer handoff, representative performance, and Linux/Windows/package evidence |
...
4. Activate worker schema v2 in the desktop adapter and hand the already-validated native VIS-2 scene to the same-window renderer with an explicit unavailable/fallback state. Then implement VIS-3/VIS-4 before calling USE-3 complete; do not replace analysis output, infer centered stock placement, or move geometry buffers into the WebView.
