...
| VIS-4 governed stock revision — In Progress | A pure final-X/Y/Z engine now creates immutable review-only revisions with complete original proposal/settings/envelope basis and recomputed stock/material/cutting values | Fourteen analytic tests pass for original/previous/new evidence, actor/time/reason, enclosure/machine-fit, stale basis, no-ops, and exact arithmetic. Application latest-session sequencing, review/adoption, DTO/permission migration, numeric editor, and renderer integration remain; no drag-only workflow |
