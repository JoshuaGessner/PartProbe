...
The USE-3 engine checkpoint implements `partprobe-rectangular-stock-envelope-policy` v1.0.0 and the distinct internal `partprobe-developer-estimate-input-proposal` v1.0.0 in `crates/setup-planner`. ABI-v4 supplies one-solid exact STEP AABB evidence. The internal proposal resolves an exact schema-v2 starter Draft into model-sensitive blank, material mass/cost, machine-fit, and coarse-runtime evidence; `partprobe-developer-estimate-proposal-adoption` v1.0.0 requires explicit review, named limitation acceptance, quantity, native actor/time, and reason before the existing deterministic engine runs. Contract v11 preserves v10's path-free evidence/exclusion trace and adds only a path-free Model & stock workspace command. Analytic/native cube/prism tests prove model sensitivity. VIS-1 phases A/B now prove the isolated native GPU scene, visual language, and a live developer-gated same-window Metal screen with return navigation and resize. Child-WebView API acceptance/replacement, governed display data and stock placement remain open. Final packaged/visual validation, governed catalog replacement, standard-size/availability, detailed runtime/adders, and ABI-v4 Linux/Windows refresh remain next.

### Model and stock visualizer/editor increment

The source-bound application viewer remains a planned USE-3 product increment, not a decorative preview and not implemented by contract v11's workspace control. It remains inside the existing architecture: the worker creates a separately versioned and bounded display derivative; the native `model-viewer` component grows from its synthetic same-window spike into ownership of source-bound GPU buffers and interaction; the application owns analysis/proposal/candidate identity and validation; and the WebView receives only path-free textual state and typed edit intents. Raw CAD, paths, and vertex/index buffers do not cross into the WebView.
...
| VIS-2 governed display derivative — In Progress | The native-only manifest/framing/decoder bind source and analysis identity, profile, bounds, layouts, references, chunks, hashes, and limits. Worker schema v2 preserves v1 analysis while adding exact optional display request/response fields. The supervisor claims separate fixed files under one shared quota, removes unreported/partial output, and the application can retain only validated native arrays plus diagnostics. The real worker currently returns explicit display unavailability | Complete: manifest/artifact rejection suites, protocol migration, shared-quota dual claim, application seam, and real-worker negative integration. Open: native cube/prism tessellation/emission, real artifact decode, desktop activation, and renderer handoff |
...
3. Finish the live-test UI cleanup, source-controlled Huntsville test baseline, fresh macOS package, and human rehearsal for contract v11 without changing `geometry-snapshot-v1` or treating the synthetic viewer as source evidence.
4. Complete VIS-2 native cube/prism tessellation and display emission, exercise real supervisor/application decoding, activate schema v2 in the desktop adapter, and hand the validated scene to the same-window renderer. Then implement VIS-3/VIS-4 before calling USE-3 complete; do not replace analysis output, infer centered stock placement, or move geometry buffers into the WebView.
